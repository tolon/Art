...
/// An empty or whitespace-only path means the source document gave a mirror
/// (or Aminet) but no file to fetch from it — a catalogue gap, not something
/// `resolve` can paper over with an empty `PackageMeta::name`.
...
mod tests {
    ...
    /// that is empty must refuse by name rather than resolve.
    #[test]
    fn an_aminet_entry_with_an_empty_path_is_refused_by_the_entrys_own_name() {
        let e = entry(
            "empty-aminet",
            EntrySource::Aminet {
                path: String::new(),
        ...
        match super::resolve(&e, &aminet(), &[]) {
            super::Resolution::Refused { why } => {
                assert!(why.contains("empty-aminet"), "got: {why}");
            }
            super::Resolution::Fetchable { .. } => panic!("no file path was given"),
        }
    }
