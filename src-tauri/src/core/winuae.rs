//! WinUAE detection and `.uae` configuration generator (Phase 2 & Phase 16).
//!
//! Detects local WinUAE executable installations and produces accurate `.uae`
//! configurations from Amiga hardware profiles. Launching the emulator itself
//! is `tools::winuae_launcher`'s job (ART-274): a process spawn is a
//! platform-specific act, and this module — like every other `core/` module —
//! must never make one (CLAUDE.md, "The core independence rule").
