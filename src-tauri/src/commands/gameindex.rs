...
///
/// **The mapping from `KickstartNeed` to `WantedImage`**
/// (`core::cardos::kickstarts::wanted_images`) **lives in `core/`, not
/// here** — `core::rom::offer` declares its own record and does not import
/// `core::gameindex`, the layering rule CLAUDE.md states with `core/rom` as
/// its own example, but `core::cardos` may import both, so it is where the
/// translation belongs. Moved from this command (ART round 3, task 9)
/// because the card build's own `core::cardos::kickstarts::find_title_needs`
/// needs the same mapping without going through the command layer; this
/// command now just calls it, the way `commands/preload.rs::rom_pairing_for`
/// still does its own translation for `core/rom/pairing`.
///
...
pub fn kickstart_offers_for(
    ...
    Ok(crate::core::rom::offer::offer_for(
        &crate::core::cardos::kickstarts::wanted_images(&need),
        &collection,
    ))
}
