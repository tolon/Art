...
- **zip** — ZIP reading, deflate only (MIT), with `flate2` (MIT / Apache-2.0),
  ...
  Apache-2.0) and `memchr` (MIT / Unlicense) beneath it. Compression and the
  other decompressors are switched off — ART only reads, and every feature
  left on is more code parsing a hostile file. **One exception, and it is not
  a feature flag**: the crate carries legacy ZipCrypto *decryption* ungated
  (only AES sits behind `aes-crypto`), and ART uses it for one job — opening a
  BoingBag's own payload with the key its recipe carries (see "Derived data")
...
- **libpfs3** — PFS3 (Professional File System III) reading and writing, the
  ...
  reasoning and its cost. Vendored and patched as `0.1.3+art.3` in
  `src-tauri/vendor/libpfs3` (ART-310 to ART-316; `ART-PATCH.md` there says what changed)
...
ART ships a few facts it did not measure itself:
...
- **The two BoingBag payload keys**, in
  `src-tauri/src/core/osinstall/recipes/packages/boingbag-39-1.json` and
  `…/boingbag-39-2.json` (`payload_password`). Adopted, not derived by ART,
  from [`rootrootde/emu68hatcher`](https://github.com/rootrootde/emu68hatcher)
  (**MIT**), file
  `src/main/python/emu68hatcher/builder/staging/boingbag.py`, read 2026-09-08 —
  which in turn credits **Emu68-Imager**
  ([`mja65/Emu68-Imager-Software`](https://github.com/mja65/Emu68-Imager-Software),
  **MIT**), the project its BoingBag staging is derived from. Both values are
  published in that source; ART copies them and derives nothing.

  What they are for: Haage & Partner ship each BoingBag's payload as a
  ZipCrypto ZIP inside the wrapper LHA, and ART opens it to place the update
  from Windows instead of running the package's own `Updater` inside an
  emulator (ART-166, the owner's decision of 2026-09-08). **No BoingBag
  content is redistributed** — the user supplies their own archive, and ART
  ships only the two key strings and the rules saying where each of the
  payload's own files goes. `core/archive/zip.rs` uses a key it was handed and
  nothing else: it derives none, tries no second one, and refuses once
  (`ART-PAYLOAD-PASSWORD`) when the one it carries does not fit.
