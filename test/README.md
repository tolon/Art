...
| ZIP / 7z / LHA | ART's own tests, and the one hostile-archive test every backend is run through | an outside implementation, per archive |
| **C64 disks and tapes** | ART's own tests and fixtures only | **anything outside ART.** No independent reader has checked a D64 or T64 that ART listed |
...
anyone who can point a `c1541`, VICE or DirMaster at a D64 ART has listed.
