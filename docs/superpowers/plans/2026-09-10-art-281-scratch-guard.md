...
### Task 7 (added 2026-09-10 after Task 5): the two shapes the helper census missed

The 63 helpers were the dominant cause, not the whole of it. Reading the tree after the batches landed shows two more shapes that leave `art-*` entries under the scratch root, and the whole-suite fixed arm cannot reach 0 while they stand:

- **7a — inline joins.** ~45 sites in test bodies or helpers not named `scratch` build `std::env::temp_dir().join(format!("art-…-{}", test_scratch_id()))` (or a fixed name) by hand: `core/lha/mod.rs` (`tmp()` and four inline), `core/adf/create.rs`, `core/analysis.rs`, `core/artwork/{cache,enrich}.rs`, `core/cbm/{d64,mod,t64}.rs`, `core/detect.rs`, `core/gotek.rs`, `core/hdf.rs:311`, `core/iso/mod.rs`, `core/pistorm/mod.rs`, `core/preload/amiga_names.rs`, `core/sources/{cache,catalog/jsonl,fetch,installed,library,mirror}.rs`, `core/volume/journal.rs` (two), `core/workflow/{builtin,registry}.rs`, `commands/{artwork,card,gameindex,iso,layout}.rs`, `tools/hst_imager.rs`. Each becomes `let (_guard, dir) = crate::core::ScratchDir::pair("<the literal's prefix>", "<tag>")` (a fixed name gets its own tag). Helpers like `tmp(tag)` convert like a `scratch` helper. `scripts/scratch-guard-sweep.py` gains **rule 4**: in test code, `temp_dir().join(` on a line that is not inside `ScratchDir`'s own impl is an offender "builds a scratch path by hand" — except a line that only reads (`read_dir(`, `assert`, `exists()` on a name the test never creates).
- **7b — product roots.** ~75 tests hand `&std::env::temp_dir()` to product code as *its* scratch root (`build_plan`, `prepare_archives`, `unpack_for_install`, `apply_staging_in`, `open_nested`, `preview_component_collisions`, `launch_winuae`, …): `core/whdload/install.rs` (26), `commands/osinstall.rs` (27), `commands/archives.rs` (22), `core/osinstall/source_archive.rs` (13), `commands/sources.rs`, `commands/amigainstall.rs`, `core/sources/install.rs`, `core/osinstall/{apply,plan,scan}.rs`, `tools/winuae_launcher.rs`. Product staging lands under that root and no sweeper runs in a test. Each test binds `let (_guard, root) = ScratchDir::pair("art-<module>-root", "<tag>")` once and passes `&root`; a test that asserts on the root's contents (`read_dir(temp_dir())`, `scratch_root_for(&temp_dir())`, the namespace tests in `commands/osinstall.rs` ~3700-3760, `sweep_stale_preview_scratch_dirs`) keeps `temp_dir()` where the assertion is about the platform root itself — say which in the report. Rule 5: in test code, `&std::env::temp_dir()` as a call argument is an offender "hands the platform root to product code".

Both measure as before, by names: list the entries under the root before and after the module's run (names, not only counts), defective and fixed arms. 7a owns the sweep (rules 4 and 5) and every file in its list; 7b owns its list; `commands/sources.rs` and `core/hdf.rs` go to whichever list names them (7b, 7a). They run concurrently; commits name their paths.
