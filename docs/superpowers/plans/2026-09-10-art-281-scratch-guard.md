...
- [ ] The whole-suite experiment, both arms: **defective arm** = `git stash` is forbidden, so use a worktree: `git worktree add ../art-281-baseline main`, in it `scratch-residue.py` → `N0`, `cargo test --lib` (unpiped, to a file; quote `test result:`), `scratch-residue.py` → `N1`; **fixed arm** on the branch the same, `N2` → `N3`. Expected: `N1 − N0` in the thousands, `N3 − N2 = 0` (or the handful the `Drop` reported on stderr — grep the log for `ScratchDir:` and quote every line). Run the fixed arm **twice** (ART-059). Remove the worktree afterwards (`git worktree remove`).
