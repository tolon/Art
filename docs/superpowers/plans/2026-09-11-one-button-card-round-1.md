...
2. **Content** — `core/card/content.rs`: classify a source (folder · archive · WHDLoad HDF · ADF · unusable); measure it into a `ContentMeasure` (and its non-ASCII names); prepare it into a staging directory (archive gate; a WHDLoad HDF's drawer + `.info` via `extract_from_volume`, the boot scaffold left behind). `core/whdload/system.rs`: WHDLoad from the material by `$VER:` into a tree; Kickstarts via `rom::offer`/`rom::place`. `core/preload`: a partition fed by several folders.
