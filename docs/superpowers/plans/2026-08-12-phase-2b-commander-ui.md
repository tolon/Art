...
| 11 | Two tabs on the right pane survive a restart with their paths and sort orders | 🟡 **Half verified, after the fact.** Two tabs made in the running pane do reach `settings.json` as two tabs, with their paths, the focused side and the command history — the **write** half is real. The **read-back** half still has not been seen: nothing has closed and reopened ART since. The round-trip itself now has 9 tests (`src/lib/paneSession.ts`), added specifically because this row said it had none |
