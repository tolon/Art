...
| Format Amiga volumes and fill them | SD-2 G3 route E | ✅ | `core/preload/` (the plan, the trait and the runner) + `tools/hst_imager.rs` + `commands/preload.rs`. Run against the real tool on a card ART built: the RDB embedded, `DH0` formatted as `Work`, a tree copied in, and the volume listed back by the tool as 1 directory, 2 files, 20 B. Refuses before anything runs when a partition names a filesystem the card lacks (ART-084 again). ART has **no PFS3 reader**, so nothing inside such a volume can be checked by ART |
| Preparing volumes reaches the UI | SD-2 G3 route E | ✅ | `src/lib/preload.ts` + the OS Builder's **prepare a card's Amiga volumes**, its third kind. §92 in full: read the card → choose what gets erased → preview → confirm the count → run as a job → report. **Nothing is chosen to start with and the choices are not remembered**, unlike every path on the screen — a screen that came back armed to erase two partitions would be ART's own remembering rule turned into a hazard. The volume-name rules are `check_name`'s, checked before the format rather than during it. The result panel says ART cannot read the volume back (§89). `hst.imager`'s path is a setting beside `winuaePath`; ART does not ship it |
...
| OS install: put the tree onto a PFS3/FFS volume | SD-2 G5 | 🟡 | `core/preload::NativeFormatter::copy_in` (`format_partition` then `copy_in`, the same two calls G5 always makes) — proven against the real tree, but **not the whole tree**: 3061 of 4030 files and 224 of 330 directories (10.3 MB of 11.9 MB) written and independently confirmed — `hst-imager fs dir -r` reporting the identical count, and 3059 of 3061 extracted files hashing byte-for-byte against the source (the 2 misses are [ART-114](ISSUES.md), not this feature). The missing quarter — **969 files, 106 directories** — is every AmigaDOS name outside plain ASCII, excluded because **any non-ASCII AmigaDOS name fails the copy outright**: `libpfs3` 0.1.3 writes a name as UTF-8 and reads it back as Latin-1 ([ART-113](ISSUES.md)), and this is real content on the user's own media (`Locale-ES/-FR/-PT/-TR` and the base `Locale` disk's own country files: `español`, `français`, `português`, `türkçe`, `österreich`). **The 3061/224/10.3 MB, 3059/3061 and 969/106 figures above are a one-off measurement**: they came from a manual Python pass, not a committed script, so a later session cannot rerun it to reproduce them (see [ART-113](ISSUES.md)). **Reachable from the product as of [ART-120](ISSUES.md), fixed 2026-08-16**: `commands/preload.rs::run_with_fallback` tries `NativeFormatter` first for every step and only reaches `hst-imager` for the two known capability gaps this row itself is about — the non-ASCII names above ([ART-113](ISSUES.md)) and embedding a driver into a foreign card's existing RDB ([ART-117](ISSUES.md)) — refusing clearly, naming both the missing tool and the reason, when the fallback is needed and no `hst.imager.exe` is configured. Which tool ran which step, and why, is reported per step (`StepReport`), never silently. Not re-run against the real 4030-file tree under the new fallback path; the 969/106 non-ASCII figures above are therefore still the last real measurement of that gap, not a claim that the fallback closes it end to end. The osinstall→preload handoff is still two separate manual steps, not one flow |
