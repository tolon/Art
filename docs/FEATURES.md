...
| Place the Emu68 payload on FAT32 | SD-1 G2 | 🟡 | the engine lays down whatever files it is given, folders included — proved with the **real** `Emu68-pistorm.zip` (20 files, `overlays/`) plus a Kickstart. What is **not** built is choosing them: unpacking the archive itself, picking the kernel for the user's board and release line, naming the Kickstart `kick.rom` |
