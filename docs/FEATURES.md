...
| Play a catalogued title in WinUAE | collection-wave-c §4 | 🟡 | `core/launch/` (plan, `.rp9` disk/hardfile extraction, the Y2 boot directory) plus `core/winuae.rs`'s `DirMount` lines, reached from the panel's Play button. Picks a machine profile from the catalogue's chipset and a Kickstart from the ROM folder, **refusing rather than launching** when none suits; a bare `.adf`/`.rp9` floppy set mounts and boots directly, a hardfile mounts read-only, a WHDLoad drawer gets Y1 (mount the user's own system read-only, hand over to them) with Y2 (ART's own one-click boot directory) on top of it, never removed from the panel. Every launch goes through the operation log. Two Critical findings from review, both fixed before landing: an `.rp9`-packaged HARDFILE title was mounting the archive itself, writable ([ART-141](ISSUES.md)), and a WHDLoad slave name was refused for quoting/redirection characters but not `;`, `` ` `` or `$`, all of which the AmigaShell command-line parser accepts. **Run for the first time 2026-08-18, against a real title (`1000 Miglia`) in a real WinUAE**: Y2 landed the user at an AmigaDOS CLI, not the game — ART's boot directory has no `C` drawer, so `C:` was never auto-assigned and the script's own first `Assign` line could not run ([ART-145](ISSUES.md), fixed same day). **The fixed script has not yet been re-run against the real AmiKit image** (`E:\amiga\amikit\AmiKit.hdf`) — whether `1000 Miglia` now actually starts is still open. **An `.rp9` floppy title, however, now runs for real**: *3D Demo* launched on the same day and the game started — ART detected the package, extracted its two floppies **in order** into its own launch directory (`launch/3d-demo-491e8a16/3ddemo1.adf`, `3ddemo2.adf`, read off disk rather than reported), chose a machine and a Kickstart, wrote the configuration and started WinUAE. That is the plan, the extraction, the multi-disk order, the ROM choice and the launch, end to end. Y1, a bare `.adf` and an `.rp9` **hardfile** title remain unrun |
