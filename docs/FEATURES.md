...
| Place the Emu68 payload on FAT32 | SD-1 G2 | 🟡 | the engine puts named files on the partition and proves it with an independent reader. What is **not** built is choosing them: unpacking the user's `Emu68-pistorm.zip`, picking the kernel for their board and release line, naming the Kickstart `kick.rom` |
