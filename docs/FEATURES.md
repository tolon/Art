...
| OS install: build a distribution tree from the user's own AmigaOS 3.2 media | SD-2 G5 | ✅ | `core/osinstall/{scan,plan,apply}.rs` — install ADFs found by the volume name recorded *inside* them, never by filename; which components switch on (required, chosen, or a satisfied `Condition` such as pre-V47 `modules-a1200`) resolved once and shared by the preview and the build; file-level destination collisions across two disks refused by name; the tree written with a `.uaem` sidecar per file and a `distribution.json` manifest (path, component, medium, SHA-256, protection, per file). 145 `core::osinstall` unit tests. Run for real against the user's own 36-ADF 3.2 set and the paired Kickstart: 26 components on (`modules-a1200` among them, switched on by its own condition against the real V40 ROM, never chosen), 4030 files / 330 directories / 11.9 MB, zero refusals — after two recipe defects the real run found were fixed ([ART-111](ISSUES.md), [ART-112](ISSUES.md)) |
| OS install: put the tree onto a PFS3/FFS volume | SD-2 G5 | 🟡 | `core/preload::NativeFormatter::copy_in` (`format_partition` then `copy_in`, the same two calls G5 always makes) — proven against the real tree for every AmigaDOS-ASCII name: 3061 files / 224 directories / 10.3 MB written, `hst-imager fs dir -r` reporting the identical count independently, and every one of 3059 extracted files hashing byte-for-byte against the source. **Any non-ASCII AmigaDOS name fails the copy outright** — `libpfs3` 0.1.3 writes a name as UTF-8 and reads it back as Latin-1 ([ART-113](ISSUES.md)), and this is real content on the user's own media (`Locale-ES/-FR/-PT/-TR` and the base `Locale` disk's own country files: `español`, `français`, `português`, `türkçe`, `österreich`). The osinstall→preload handoff is also two separate manual steps today, not one flow |
