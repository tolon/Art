...
| Format the FAT32 boot partition | SD-1 G2 | ⏳ | the other half of G2: the table is written, the filesystem in it is not. Route decided in the gap analysis — a maintained MIT/Apache crate, not hand-rolled |
| Place the Emu68 payload on FAT32 | SD-1 G2 | ⏳ | kernel, `kick.rom`, `config.txt`, `cmdline.txt`. Every material for it has arrived; blocked only on the formatter above |
