...
`src-tauri/vendor/libpfs3` (its own `Cargo.toml` says which `0.1.3+art.N`; reached through `[patch.crates-io]`
while `Cargo.toml` still pins `=0.1.3`). Which files differ from the release, and why, is
`ART-PATCH.md` beside it, item by item — it also says how to re-vendor
