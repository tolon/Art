...
| `libpfs3` | PFS3 filesystem implementation — writes and reads the volumes AmigaOS install (SD-2 G5) puts on a PiStorm card | **LGPL-3.0-or-later** (weak copyleft; the one non-permissive dependency in `core/`, compatible with ART's GPL-3.0-or-later but noted deliberately — `core/` is meant to be promotable to a standalone crate, which is exactly the reuse the project otherwise avoids constraining) **Vendored and patched** since ART-310: `src-tauri/vendor/libpfs3`, version `0.1.3+art.2`, only `src/format.rs` (ART-310) and `src/writer.rs` (ART-312) changed (its `ART-PATCH.md` says what and why); upstream's `LICENSE` and the full LGPL-3.0 text (`COPYING.LESSER`) travel with it. |
