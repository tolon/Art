...
000000000. **ART-310 is fixed on `art-310-libpfs3-format`, not merged (2026-09-13; tasks 1–3 and 5 on the CachyOS Linux machine, task 4 on the owner's Windows machine).**
    ...
    - `plan_card_image`'s "never `NativeFormatter` past MAXSMALLDISK" rule is gone: the **format** is right at every size. ART-311 (the writer's index-block gaps) is filed and unscheduled.
    - **Not yet safe to fill a planned card natively: the writer is still 0.1.3's.** The Windows machine's own, earlier ART-310 run (2026-09-11/12, local branch `art-310-windows`, never pushed) found a writer defect that hands one anode number out twice in one operation and silently corrupts files (3 of 22 000), fixed there in `9c7c845`, and grew small-mode index blocks on demand (`de26e58`). Neither is on this branch; ART-311 records both. The owner chose on 2026-09-13 to finish this branch first and port that work afterwards — the double allocation still needs its own `ART-NNN`.
