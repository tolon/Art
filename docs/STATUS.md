...
an object links still name, and `create_hardlink` are refused by name. Still open, all unreachable from ART:
[ART-324](ISSUES.md#open) (sizes of 4 GiB or more lose their high bits in the writer), [ART-325](ISSUES.md#open)
(the crate's extra-field parser is not pfs3aio's layout), [ART-326](ISSUES.md#open) (a second entry under an
existing name), [ART-327](ISSUES.md#open) (`rename_in` drops a moved entry's comment, date and extra fields).
...
**Next:** a scoped re-review of the fix wave and ART-323, then the merge, the push and the 0.9.4 build — all the
owner's word.
