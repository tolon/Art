...
| **Build** | PASS |
| **Tests** | 1875 Rust passed, 0 failed, 20 ignored (real-media hooks, env-gated — see below) — **but not on every run**: [ART-164](ISSUES.md) makes `core::iso`'s tests share a scratch directory, and one full-suite run in about eleven fails on an arbitrary one of them (re-measured 2026-08-19: the module run on its own failed 5 times in 40, on four *different* tests). It is a test-fixture defect, not a product one, and it is the next thing that should be fixed; 637 frontend passed, 0 failed, **exit code 0** — which it had not been: `pnpm test` printed `Tests 619 passed` and exited 1, because ten jsdom tests left unhandled promise rejections behind, and CI runs that command as a blocking step ([ART-163](ISSUES.md), fixed, with the underlying product defect filed and fixed separately as [ART-165](ISSUES.md)) |
...
| **i18n** | `en.json` and `tr.json`, 1649 leaf keys each, parity enforced by `pnpm test` |
