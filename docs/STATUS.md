...
| **Build** | PASS |
| **Tests** | 1415 Rust passed, 0 failed, 4 ignored (real-media hooks, env-gated — see below); 533 frontend passed, 0 failed |
| **Clippy** | clean at `-D warnings` |
| **TypeScript** | clean |
| **Kickstart table** | 154 dumps, generated from amitools' Remus split database and verified against it on every CI run (`scripts/rom-table-check.py`, ART-104). 24 of the user's own collection now named *with their machine*, including the two 40.68 builds told apart; ART's previous ten hand-listed hashes matched none of them |
