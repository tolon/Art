...
| **Build** | PASS |
| **Tests** | 1489 Rust passed, 0 failed, 9 ignored (real-media hooks, env-gated — see below); 560 frontend passed, 0 failed |
...
| **i18n** | `en.json` and `tr.json`, 1479 leaf keys each, parity enforced by `pnpm test` |
...
- **The user has asked for a larger Collection, and it is a design round of
  its own rather than a wave-2 task.** In their order:
  1. **The catalogue is not saved.** Every visit re-scans 1699 files. This was
     in the design round ("cached in ART's own data directory, refreshed by a
     visible Rescan") and did not make it into wave 1's plan — an omission,
     not a decision.
  ...
  4. **More than one folder** in one catalogue. The user keeps titles in two
     places (`E:\amiga\Amigatolon\WHDload`, `E:\amiga\Titles`).
