...
000000000. **ART-310 is fixed on `art-310-libpfs3-format`, not merged (2026-09-13; tasks 1–3 and 5 on the CachyOS Linux machine, task 4 on the owner's Windows machine).**
    ...
    - **Next:** the final whole-branch review, then the owner's word to merge; then port `art-310-windows`'s writer fixes. **Owed by a person:** a patched large volume mounted under real pfs3aio in WinUAE, which is also what the prepared upstream patch waits for.
