...
`Writer::set_entry_date`). `VolumeWriter::open` and `create_blank_adf` are `#[cfg(test)]`, closed by
`core_makes_amiga_dates_only_through_the_clock`. `.uaem` dates, `volume_attributes`'s `date_text` and
...
is read again once); and the gate — `VolumeWriter::open` and `create_blank_adf` became `#[cfg(test)]`, closed by
`core::independence::core_makes_amiga_dates_only_through_the_clock`, so product code cannot make a date without
the clock. `architecture.md`'s trait table gained a sixth instance. **Suite of record:** `cd src-tauri && cargo
...
survivor:** on a UTC CI runner, `offset_at` returning 0 passes every non-ignored test — a weak guard on such a
machine; the ignored `agrees_with_dotnet_timezoneinfo_across_seasons_and_years` catches it on a non-UTC one, per
Task 2's own report, but was not re-run as part of this close-out. **Still owed by the owner:** run the ignored
.NET oracle test itself —
`$env:TMP='E:\amiga\ProjeART\build\tmp'; $env:TEMP=$env:TMP; $env:ART_TZ_ORACLE='1'; cargo test --lib
tools::local_time -- --ignored` — and mount a volume ART wrote under real pfs3aio in WinUAE, which has not been
tried under this fix. **Next:** the final whole-branch review's verdict; the merge, on the owner's word; ART-319 if the
