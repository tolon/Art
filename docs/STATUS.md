...
**The one-button card's round 2 is done on its branch, `art-card-round-2` — not merged, not pushed; the final
whole-branch review's fix wave has landed (`952f217`) and it awaits a scoped re-review of `44e1317..952f217`** (plan `docs/superpowers/plans/2026-09-16-one-button-card-round-2.md`, ledger and
...
**Next, after the scoped re-review and a merge on the owner's word: round 3** — the WHDLoad phase, `card_os_prepare` /
