...
0000000000. **ART-312 is fixed on `art-312-anode-reuse`, not merged (2026-09-13).** `libpfs3`'s writer resolves the anode index through its own pending writes, so one operation that allocates twice gets two different anode numbers; the vendored crate is `0.1.3+art.2`. A test red on 0.1.3's writer at `D009/F00070`, two mutations killed, `cargo test --lib` `test result: ok. 3258 passed; 0 failed; 58 ignored; 0 measured; 0 filtered out` twice, clippy clean. Filling a PFS3 partition natively is now bound by ART-311's ~21 500-anode ceiling rather than by silent corruption. **Next:** the package from this branch, then the owner's word to merge.
