...
2. **`ğ` and `ı` drawn, photographed.** `ş` is done: the same window title
   read `600.4GB boþ` under `topaz.font` and `600.4GB boş` under
   `TOPAZ-ISO9`, one variable, both arms captured. `ğ` and `ı` appear in arm
   1 only (`Ekran Baþlýk Çubuðu`, `kullanýmda`) — arm 2's frame clips them.
   The same font table serves all three and there is no reason to expect a
   difference, which is exactly why it is worth one more frame rather than an
   assumption. Getting there also found and fixed [ART-225](ISSUES.md).
