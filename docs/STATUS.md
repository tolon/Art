...
test --lib` `test result: ok. 3316 passed; 0 failed; 59 ignored; 0 measured; 0 filtered out` twice, identical;
...
WinUAE, which has not been tried under this fix. **Next:** the final whole-branch review's verdict; the merge, on the owner's word; ART-319 if the
owner schedules it. Reports and the spec: `.superpowers/sdd/2026-09-14-debt-5-art-317-local-time/`.
...
protection bits. **Next:** the final whole-branch review's verdict (task 9); the merge, on the owner's word;
ART-319 if the owner schedules it. **ART-317 (plan 5) is now fixed too — see the item above.** **Still owed by
