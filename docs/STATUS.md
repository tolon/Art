...
| **Tests — Rust** | `cd src-tauri && cargo test --lib` — the **full** suite — on the 0.9.1 release head, 2026-09-09: `test result: ok. 3181 passed; 0 failed; 53 ignored` (run twice, ART-059) |
...
0. **0.9.1 is released** (`v0.9.1` at `7ee91fa`, merge `7ee91fa`, 2026-09-09; release run 34331989539: success, published 2026-09-09 09:21 UTC with the MSI and the NSIS installer). What it carries beyond the three intake rounds: BoingBag 1 and 2 placed from Windows (the owner's decision of 2026-09-08; `.superpowers/sdd/2026-09-08-intake/bb-host-brief.md`, ART-166 Fixed with the oracle's counts), the emulator route for them removed, ART-282/284/286 fixed, ART-283/285 filed, the README and seven screenshots. Re-measured on the release head: `pnpm lint` clean, Vitest 91 / 1388, full `cargo test --lib` `3181 passed; 0 failed; 53 ignored` twice, fmt/clippy/sweeps clean. **Round A of the simplification spec is on `art-simplify-a-readout-first`, unmerged** (plan `docs/superpowers/plans/2026-09-09-readout-first.md`, report `.superpowers/sdd/2026-09-09-readout-first/report.md`): the readout first. Owed before merge: the owner drives § 4.3 on the build under `E:\amiga\ProjeART\build`. Then rounds B (one updates screen), C (three steps), D (the commander's columns, its own worktree) — and ART-281's mechanical scratch conversion. **Four-tab rewrite, rounds 1-3 landed on `art-four-tabs`** (branched from `art-simplify-a-readout-first`, so round A merges first; plans `docs/superpowers/plans/2026-09-09-four-tabs-round-1.md`, `…-round-2.md` and `…-round-3.md`, reports `.superpowers/sdd/2026-09-09-four-tabs/round-1-report.md`, `round-2-report.md` and `round-3-report.md`). **Round 1:** the lane is `hedef` + `dosyalar · secim · makine · derle`, the retired routes redirect, the strip has the `hedef` chip, the bar has the destination and *Derle'ye git*; the tabs still mounted today's panels. **Round 2 (spec § 8 row 2):** the Kickstart and the destination are tab 3, in their own `MachineTab.tsx`, both tabs asking through two new hooks (`useRomIdentity`, `useDestinationCheck`) so they cannot disagree; tab 1's per-file identity wall is behind one closed line and **the identification pass still runs** while it is closed. **Ruling to carry:** the keymap select stays on tab 1 until round 4 — its options come from the plan, and the plan is round 4's hook; `StepMakine` says so on screen and `steps.test.tsx` asserts that sentence. § 3.3's "built <date>" was cut: `osinstall_describe_tree` has no build date, so tab 3 says only what is there. Vitest 96 / 1443, lint and both sweeps clean, no Rust change (`git diff --stat main -- src-tauri` empty). Seven mutations put back, seven killed — two only after a weak guard was fixed and a missing one added (`c56a7d0`); [ART-290](ISSUES.md#fixed) filed there and **fixed in round 3**. **Round 3 (spec § 3.2, § 5, § 7, § 8 row 3), five commits `d322c35 · 328920c · c6a704e · 5827f32 · 3959242` plus this round's two:** tab 2 is **one tick list, three groups, one order** — the release's parts, the AmigaOS 3.9 updates in the material's own order, one first-boot tick — in a new `ChoiceTab.tsx`; `PackagePanel.tsx` and its 26 tests are **deleted**, and 20 catalogue keys retired from both languages. The plan computation left `OsInstall.tsx` for `src/lib/useInstallPlan.ts`, so both tabs plan through one code path, and `src/lib/useChainTree.ts` gives both tabs one tree. **ART-290 is fixed**: the component ticks are `session.components`, seeded once from `osinstall.chosen.<release>` / `osinstall.excludedConditional.<release>`, which are never written again. **Six rulings to carry:** the replaces fold counts *components* only this round, so ART-289's preview guard is round 4's and ART-289 stays open; `FirstBootPanel` is mounted by no route and the first-boot *write* is round 4's phase (a branch state, not a shipped one — no package goes out before round 4); `firstboot.wanted` is optional and absent means ticked, so rendering never writes it; the wizard has **one route**, so a row that runs on the Amiga is drawn and explained on tab 2 but never offered there; a closed-but-ticked row keeps an untick control, because refusing a *tick* is not refusing an *untick*; and refusals, the plan, the keymap, Run and the result stay on tab 1 until round 4. Vitest **98 / 1475**, lint clean, parity/literal-keys/dead-keys clean, control-byte and contrast sweeps clean, no Rust change (`git diff --stat main -- src-tauri` empty). Seven mutations put back, seven killed, no survivors, the control measured first. **Round 3's whole-branch review then landed as a fix wave** (`0792991` and the docs commit after it): tab 2's readiness banner judged `session.tree.root` while the list under it worked on `useChainTree`'s tree — one tab, two answers — and both its sentences told the user to pick a tree "below", where the picker deleted this round used to be; they name tab 3 now, and `readiness()` takes the root it is judging. The Amiga-side panel's *Distribution tree* Browse was dead whenever the destination won (it wrote a value `useChainTree` overruled, so the path snapped back): the hook says which rule produced the tree and the panel draws one sentence instead of a field that cannot change it. Six mutations put back, six killed, no survivors. **Three sentences on tab 2 are branch-state claims round 4 must make true** — `osBuilder.choice.lead`'s "Ticked rows are written", the update ticks and the first-boot tick — listed in `round-3-report.md` § 8; nothing ships before round 4, which is what makes them safe to leave standing. **Next: round 4** — the run, the keymap and the end of `OsInstall.tsx`; plan `docs/superpowers/plans/2026-09-09-four-tabs-round-4.md`. **Then the package** — the owner's instruction is that no installer is built before round 4 is in; nothing has been built or copied for rounds 1-3. Owed by a person: tab 2 over `E:\amiga\Amigatolon\os39` with the destination pointed at an existing 3.9 tree and at an empty folder, a stale tick's untick control, and the replaces fold open and closed; tab 3 on a real screen against an empty folder, an occupied one and an ART tree; tab 1 at 100 % and 200 % with the fold closed and open; and, still from round 1, the strip check in both languages (`pnpm dev` + `scripts/osbuilder-strip-check.py`, unrun since it was edited) and typing a retired step URL into the address bar on a dev build, where the router is a hash router and jsdom's `MemoryRouter` is not. **Still owed by a person**: the chain driven on the real screen (BB1 → BB2 from Windows, then boot and `version full`). Findings list: `D:\Projeler\Amiga\ART-test-bulgulari-2026-09-07.md`.
