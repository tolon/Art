...
**The one-button card's round 2 is merged to `main` as `69d3a40` and pushed, both on the owner's word (2026-09-17;
branch deleted; CI run 35196578229 success).** The final whole-branch review's fix wave (`952f217`) passed its scoped re-review
(`fix-wave-re-review.md`: 15 ADDRESSED, mergeable), and the re-review's residuals landed in `df71f37`
(`lzx::read_selected` reports a member once; the epoch-date and `:` rule comments said right) **with ART-341 filed
open** — `core/osinstall` keeps `:` names that `check_name` refuses mid-copy. **Open from the round: ART-339, ART-340,
ART-341.** (plan `docs/superpowers/plans/2026-09-16-one-button-card-round-2.md`, ledger and
task reports `.superpowers/sdd/2026-09-16-one-button-card-round-2/`; base `dbc0566`, 12 code commits `ce8e8e7..3193e36`,
Task 12's guard fix `33d297b` and the records). The owner approved the plan and its seven decisions on 2026-09-17. **Built, core only:** ART's
own LZX reader (Tasks 2-3), Amiga protection bits, comments and dates on every archive entry (1), HDF extraction
recording escaped names as pairs and writing drawer sidecars (4), the native copy carrying a sidecar's date and
comment to PFS3 (libpfs3 `0.1.3+art.12`) and its comment to FFS (5), sizing that counts comments (6), the
`.art-amiga-names.json` record and hst-imager told to read `.uaem` (7), `core/card/content.rs` — classify, place,
measure, check, prepare (8-9), and a partition taking several sources in one copy step (10). Filed and fixed:
ART-332…ART-338; **ART-338**, the LZX reader refusing a match into the zeroed window, was introduced and caught
inside the round by Task 11's owner-material hook and never shipped. Filed open: **ART-339**, `core/card` ⇄
`core/preload` importing each other (ruling R6, accepted for the round). **Task 12's whole-suite run** caught one
CI-blocking guard the per-task module runs never reached — Task 9's DOS-date conversion named `amiga_from_wall` in
product code (ART-317's guard); fixed in `33d297b` (fields to an `AmigaDate` directly; 2 mutations killed), then
`cargo test --lib` `3609 passed; 0 failed; 64 ignored` twice, `pnpm test` 112 files / 1723 tests. Measured on the owner's material (Task 11):
`lzx-oracle-check.py` against unar, boingbag1 998/998 and WiFi 54/54 identical; the card hook staged Prehistoric
Tale and B-17 to their drawer and icon, and read WHDLoadDemos100.lha as a collection of 893 drawers. **Mutations:**
every task's named guards killed; one survivor disclosed — sizing's comment-length byte `+1` → `+0` (Task 6, cylinder
rounding) — closed by the fix wave. **The final whole-branch review** (`final-review.md` in the ledger folder) said
*with fixes*: 0 Critical, 3 Important — `./`- or `/`-prefixed archive names slipped past R7 (I1), a dateless entry's
sidecar dated 1978 reached FFS and hst-imager (I2), a names-record value holding `/` could write into a sibling drawer
(I3) — and 10 Minor. **All thirteen fixed in one wave, `952f217`** (`fix-wave-report.md`), each with a test seen red
first; the FFS comment refusal now names the file (T5), an exact-value entry-size test kills the survivor (T6);
mutations 7/7 killed; `cargo test --lib` `3626 passed; 0 failed; 64 ignored`, sweeps and lint clean. Triaged
acceptable by the review and left: a single WHDLoad drawer deeper than `MAX_SLAVE_DEPTH` is placed as a plain tree (8).
Filed open **ART-340**: a refusal after unpacking began leaves staging part-filled — round 3's command must hold
staging in a `ScratchDir`-style guard (9). **Owed, not proven:** hst-imager's `.uaem` handling on PFS3
(the measurement was FFS); an Amiga-made ZIP (every ZIP the owner has is PC-made — the Latin-1 comment reading is
tested on a hand-built one only); a comment written by libpfs3
`+art.12` read back by pfs3aio on a real Amiga; an LZX match running past a block end (never met in real material).
**Next, after a merge on the owner's word: round 3** — the WHDLoad phase, `card_os_prepare` /
`card_os_build`, `.partial`, `run_with_fallback` reachable from a card build, and the end-to-end card (design § 8.3);
every screen is round 4.
...
**0.9.4 is released** (2026-09-15, published on the owner's word). Round 2 of the one-button card, chosen on the
owner's word on 2026-09-16, is done on its branch (above). First boot's phase 4 was checked against the tree first and **most of its
...
- **The Emu68 Hatcher intake** (`rootrootde/emu68hatcher`, MIT) — scoped at six
  ...
  the one-button card's round 2 is built on `art-card-round-2` and awaits its final review; round 3 is next.
