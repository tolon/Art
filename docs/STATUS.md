...
| **Tests — Rust** | `cd src-tauri && cargo test --lib` — the **full** suite — on `art-osbuilder-intake` (`3263f80`), 2026-09-08: `test result: ok. 3202 passed; 0 failed` (run twice, ART-059). On the merged `main` (`13e6707`): 3062 |
| **Tests — frontend** | `pnpm test` on `art-osbuilder-intake` (`3263f80`), 2026-09-08: `Test Files 91 passed`, `Tests 1410 passed`. On the merged `main` (`13e6707`): 87 / 1194 |
...
0. **All three intake rounds are done on `art-osbuilder-intake` (off `main` `e49af57`), built once at the end, not merged.** The owner's decision after the research (`docs/superpowers/specs/2026-09-08-os-builder-intake-research.md`): *sıra ile hepsini yap* — round 1 (ART-275/276/277), round 2 (`…-intake-design.md`: material folder, slots, readout, guide), round 3 (`…-chain-design.md`: the chain as data and as one screen, and the measurements of § 2.3–2.4). Head `3263f80`, re-measured: Vitest 91 / 1410, full `cargo test --lib` 3202 passed, clippy/fmt/sweeps clean. The one installer to test: `E:\amiga\ProjeART\build\Amiga Retro Toolkit_0.9.0_x64-setup_intake-final.exe`. Merge/push: the owner's call, menu presented. **Still owed by a person**: the chain driven on the real screen over `E:\amiga\Amigatolon\os39` — BB1 → BB2 → a host-placed row, then `version full`. Ledger and all briefs/reviews under `.superpowers/sdd/2026-09-08-intake/` (local-only).
