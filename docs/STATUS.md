...
| **`main` / `origin`** | `main` is at `13e6707` (the Windows 11 look merged `--no-ff`, 2026-09-07 night, the owner's decision); `origin/main` is still `76eac3d`, so **eleven commits are unpushed** — the docs commit `dd69810`, the merge and its nine — until the owner says push. `origin` was last pushed 2026-09-07 evening (`32651c9..76eac3d`), and CI run 34151589760 passed on it |
| **Tests — Rust** | `cd src-tauri && cargo test --lib` — the **full** suite — on `art-osbuilder-intake` (`c7f12f7`), 2026-09-08: `test result: ok. 3158 passed; 0 failed; 52 ignored` (run twice, ART-059). On the merged `main` (`13e6707`) the same evening: 3062 |
| **Tests — frontend** | `pnpm test` on `art-osbuilder-intake` (`c7f12f7`), 2026-09-08: `Test Files 90 passed`, `Tests 1359 passed`. On the merged `main` (`13e6707`): 87 / 1194 |
...
0. **Three rounds on `art-osbuilder-intake` (off `main` `e49af57`): rounds 1 and 2 done and built, round 3 next, nothing merged.** The owner's decision after the research (`docs/superpowers/specs/2026-09-08-os-builder-intake-research.md`): *sıra ile hepsini yap*. Round 1 (ART-275/276/277) and round 2 (`2026-09-08-os-builder-intake-design.md`: ART's own 3.9 hash rows, `slots.rs`, one material folder list, the readout, the panel reading slots, the guide text) are at `c7f12f7`, each task reviewed and re-reviewed, each round reviewed whole; re-measured: Vitest 90 / 1359, full `cargo test --lib` 3158 passed, clippy/fmt/sweeps clean, `media-table-check.py` 11 of the owner's files verified. Installers copied to `E:\amiga\ProjeART\build\…_intake-r1.exe` and `…_intake-r2.exe`. Round 3 (the chain screen — `2026-09-08-os-builder-chain-design.md`; briefs `r3-task-1..3` under `.superpowers/sdd/2026-09-08-intake/`) is under way on the same branch: **task 1 is done** (the chain as data — three new recipes, `chain_position`/`superseded_by`/`not_yet_runnable`, `chain::rows_for`, `osinstall_chain` and `src/lib/chain.ts`; report `r3-task-1-report.md`), **task 2 has landed on the branch** (`5a6bd52` the chain screen with one Run button, plus fix rounds `d1e693b` and `ff43052`), and **task 3 is done, with its fix round and the whole-branch fix on top** (`r3-task-3-report.md` + its two fix sections): eight real WinUAE runs on the owner's own BoingBags. ART-227's three remaining fix-ups measured — `C/Installer` byte-identical and locale catalogs 0 of 597 touched → *measured, not needed*; `XAD-Update` genuinely missing (`xadmaster` stuck at 9.1) and **built** as [ART-280](ISSUES.md#fixed): a typed, version-gated `follow_ups` declaration emitted as four more lines in the one boot script, HstWB's own shape, reporting separately in `art-followup.txt` (`ran`/`not-needed`/`failed`/`not-checked`) beside the ending and never inside it. **Proved on real material:** `xadmaster.library` 9.1 (105 368 B) → **`10.0 (31.03.2001)`, 110 100 B**, `follow-up: Some(Ran)`, 156.6 s. The `Updater` experiment answered: a corrupt payload **hangs** rather than warning (`TimedOut` 1 800.7 s, twice) and a wrong target returns **`ok`, twice, promoted** with `version.library` at exactly the right 45.3 — so the planned `leaves_version` check and fifth `NotApplied` ending would have passed a tree missing 57 files and were **not** built. The narrow finding: *the version string a package updates is not evidence; a file the package changes can be* (`xadmaster` 9.0 vs 9.1 does separate those trees). The whole-branch review then found two states where the screen out-claimed the core, both now closed: the one Run button could land on the **CD row** (`medium_state` never answers `Ready` now) and on a package whose **`requires_components`** the tree lacks (`BlockedByComponent`, naming the component by the key the components step uses). The owner's rule 2 is honoured on both halves at last — the identify pass hashes an `.iso` only when its own volume name is one a shipped recipe installs from, with a fifth per-file ending naming what it left alone. `chain::refuse_unless_installable` is what protects a user, and it is a **bookkeeping** guard — it reads a manifest only a successful ART run writes. Three defects filed: [ART-278](ISSUES.md#open) (a hung installer writes unbounded output into the staged copy — 170 MB in 30 minutes, nothing bounds or notices it), [ART-279](ISSUES.md#open) (the `TimedOut` next step is wrong advice for a hang) and [ART-281](ISSUES.md#open) (**the unit suite's scratch never empties: `D:\tmp\art-tests` holds 263 484 directories / 764 GB, 28 568 of them created today** — ART-184 still running where it was moved to). Full `cargo test --lib` 3202 passed, twice; Vitest 91 / 1410; lint, clippy, fmt and the four sweeps clean. The design (`2026-09-08-os-builder-chain-design.md`) is corrected in place where the tree diverged from it. **Still owed by a person**: driving the readout and BoingBag 1 → 2 → Locale through the screens with real files — no run has yet completed through the screen. The Windows 11 look merged earlier the same night as `13e6707`.
