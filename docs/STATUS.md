...
| | |
|---|---|
| **Last updated** | 2026-08-18 (ART-149) |
...
| **Build** | PASS |
| **Tests** | 1709 Rust passed, 0 failed, 11 ignored (real-media hooks, env-gated — see below); 587 frontend passed, 0 failed |
...
| **i18n** | `en.json` and `tr.json`, 1584 leaf keys each, parity enforced by `pnpm test` |
...
| Date | Change | Tests |
|---|---|---|
| 2026-08-18 | **ART-149: `hardfile2=`'s bare-image geometry rounded almost every real hardfile down to a whole cylinder, silently dropping the rest of the file — a second, independent cause of the same "not a DOS disk" symptom ART-148 also produced against this same file.** Pressing Play on `1000 Miglia` put WinUAE at "not a DOS disk" on a fresh, correctly-classified `HardfileShape::Bare` image. ART-146's fix decided *which* hardfiles get forced geometry; it never checked whether that geometry was right. `32,1,2,512` makes one cylinder 16,384 bytes, and WinUAE floors `filesize / blocksize / (sectors × surfaces)` (`hardfile.cpp::getchs2`) to get the cylinder count, so any file not a whole multiple of 16,384 bytes loses its last partial cylinder outright. `1000 Miglia v1.2.hdf` is 1,195,008 bytes = 2334 blocks = 72.9375 old cylinders, truncated to 2304; its FFS root block at 1167 (half the real count) falls past the truncated disk's own midpoint (1152), so the layout stops validating. Not one unlucky file: 0 of 40 sampled images from the same folder were a whole number of 32×512 cylinders — wrong for essentially all 1697 of this user's WHDLoad titles. Fixed by changing to `sectors=1, surfaces=1`, so one cylinder is exactly one 512-byte block and cylinders = filesize/512 divides evenly for any valid image. `reserved` was checked against e-uae's `docs/configuration.txt` rather than assumed — it documents "typically 2" and gives a bare, non-RDB partition image as its own worked example at `reserved=2` — so it stays at 2; only the cylinder-rounding pair was wrong. One `generate_uae_config` serves both `commands/winuae.rs::winuae_launch` and the catalogued-title path, so both are fixed together; confirmed, not assumed, that ART's own `create_hdf` images (always whole megabytes, `HardDiskStudio.tsx`) were already exact under the old geometry too, since 1 MiB is 64× the old cylinder size. New test `bare_geometry_represents_a_non_round_real_image_exactly` is built on the real 1,195,008-byte measurement specifically because a round-size-only suite could not have caught this. **This fix was committed once already (`81e3162`, filed then as ART-148), reverted with `git reset --hard` on a mistaken stand-down that preferred weaker forum evidence about round-sized images over the author's own reading of WinUAE's source, and is restored here as ART-149** (ART-148 was taken in the meantime by the unrelated Kickstart-floor defect — also found against `1000 Miglia`, see [ART-148](ISSUES.md)). Still unretried: whether `1000 Miglia` now actually boots and the game runs | 1709 Rust / 587 frontend |
