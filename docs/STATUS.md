...
0. **Three rounds on `art-osbuilder-intake` (off `main` `e49af57`), round 1 done and built, unmerged.** The owner's decision after the research (`docs/superpowers/specs/2026-09-08-os-builder-intake-research.md`): *sıra ile hepsini yap* — round 1 (ART-275 cursor keys, ART-276 hash clash, ART-277 the Amiga-side panel) is at `759f88a`, reviewed three times and whole-branch, re-measured: Vitest 88 / 1258, full `cargo test --lib` 3085 passed, clippy/fmt/sweeps clean; installer copied to `E:\amiga\ProjeART\build\…_intake-r1.exe`. Round 2 (one material folder list, slots, the readout, ART's own 3.9 hash rows — `2026-09-08-os-builder-intake-design.md`, four briefs under `.superpowers/sdd/2026-09-08-intake/`) and round 3 (the chain screen — `2026-09-08-os-builder-chain-design.md`) follow on the same branch. **Still owed by a person**: driving BoingBag 1 → 2 → Locale through the panel with real files — no run has yet completed through the screen. The Windows 11 look merged earlier the same night as `13e6707`.
