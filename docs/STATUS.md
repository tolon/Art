...
**The third debt round, `art-debt-3-0915`, is unmerged and unpushed.** Its items: ART-321 (the MBR slot carried
...
and strings (item 2). Still open, all unreachable from ART: [ART-324](ISSUES.md#open) (sizes of 4 GiB or more lose
their high bits in the writer), [ART-328](ISSUES.md#open) (the writer stores new names as UTF-8), [ART-329](ISSUES.md#open)
(a moved directory keeps its old parent; a move into its own subtree is not refused).

**Item 5 is unproven.** The action tags were checked against each tag's own `action.yml` and our inputs, not run:
CI proves it on the first run after the push, on `windows-latest`, and `release.yml` only on the first release —
read that first CI run before tagging 0.9.4.

**Next:** one scoped re-review of ART-325–327 and the re-review's items (`a9fd8c3..` the branch head), then the
merge, the push and the 0.9.4 build — all the owner's word.
After that, the next code work as last recorded: first boot's phase 3 (the reboot — `C:Reboot` is absent on both
AmigaOS 3.9 trees, so it must be checked for, as phase 2 checks for `C:Sort`), and round 2 of the one-button card
(`core/card/content.rs`, its own plan).
