...
0. **0.9.1 is released** (`v0.9.1` at `7ee91fa`, merge `7ee91fa`, 2026-09-09; release run 34331989539: success, published 2026-09-09 09:21 UTC with the MSI and the NSIS installer). What it carries beyond the three intake rounds: BoingBag 1 and 2 placed from Windows (the owner's decision of 2026-09-08; `.superpowers/sdd/2026-09-08-intake/bb-host-brief.md`, ART-166 Fixed with the oracle's counts), the emulator route for them removed, ART-282/284/286 fixed, ART-283/285 filed, the README and seven screenshots. Re-measured on the release head: `pnpm lint` clean, Vitest 91 / 1388, full `cargo test --lib` `3181 passed; 0 failed; 53 ignored` twice, fmt/clippy/sweeps clean. **The simplification spec's round A and the four-tab rewrite are one branch, and rounds 1-4 have landed on it.** `art-four-tabs` was cut from `art-simplify-a-readout-first` at `db59c78`, so round A's readout-first work is in this branch's history (`git merge-base --is-ancestor` confirms it, 2026-09-09) and the two reach `main` in one `--no-ff` merge — there is no separate branch waiting. Plans `docs/superpowers/plans/2026-09-09-readout-first.md` and `docs/superpowers/plans/2026-09-09-four-tabs-round-{1,2,3,4}.md`; reports `.superpowers/sdd/2026-09-09-readout-first/report.md` and `.superpowers/sdd/2026-09-09-four-tabs/round-{1,2,3,4}-report.md`.

    **What the lane is now: `hedef` (the entry chip) + four numbered tabs.** One line each:

    - **Tab 1 `dosyalar` — the material.** One folder list for the whole build, the readout first, the per-file identity wall behind one closed line (the pass still runs while it is closed), `AmigaInstallPanel` still at its foot until round 5 — and since this round it **plans nothing at all**: `useLayers(release)` reads the shipped recipe, and `osinstall_plan` and `osinstall_components` are never called from it.
    - **Tab 2 `secim` — one tick list, three groups, one order.** The release's own parts, the AmigaOS 3.9 updates in the material's own order with the chain's own sentence per row, and one first-boot tick; below them the folded *what this would replace* count.
    - **Tab 3 `makine` — Kickstart, keymap, destination.** The keymap select joined it in this round (round 2 had deferred it because its options come from the plan).
    ...
    **`OsInstall.tsx` is deleted** (2 460 lines at the start of the rewrite; `git mv`'d to `FilesTab.tsx`, 1 256 lines at the end of round 4). `PackagePanel.tsx` went in round 3. **`FirstBootPanel.tsx` is mounted by no route** and keeps its **21** tests (not 18 — that number was carried forward twice and is corrected here); round 5's WinUAE studio decides its future. **ART-289 and ART-290 are Fixed.** Round 4 measured: `pnpm lint` clean, Vitest **103 / 1570**, parity/literal-keys/dead-keys clean, control-byte and contrast sweeps clean, **no Rust change** (`git diff --stat main -- src-tauri` empty), and spec § 10.6's one-off grep re-run — the workflow catalogue still points at `/os-builder` only. Eleven mutations put back, eleven killed, the control measured first; the one survivor was a weak guard and got the case it was missing (`bdef944`), not a disclosure.

    **Package: not built by round 4 itself.** The ruling is that the installer is built and copied to `E:\amiga\ProjeART\build\` **after the whole-branch review and its fix wave**, so the owner's package is the reviewed tree; the path goes here when it exists.

    **What a person drives before this is called done** (spec § 7, and it is all still owed): from a clean destination tick **BoingBag 1, BoingBag 2, the Locale update and the Türkçe catalogs**; **one press** on tab 4; **five phase lines in order**; then **boot the tree under WinUAE and ask it `version full`** — ask the artefact. Then the **update-mode run** on an existing AmigaOS 3.9 tree (no tree phase, no refusal at phase 1 — the round's central ruling, never driven). Then `python scripts/osbuilder-strip-check.py` with `pnpm dev` running, **in both languages**, unrun since round 1 edited the lane. Then tab 3 against an **empty folder, an occupied one, an ART tree and a missing one**. Standing with them: `/os-builder/paketler` typed into a dev build's address bar (a hash router, which jsdom's `MemoryRouter` is not), tabs 1 and 2 at 100 % and 200 %, and a stale tick's untick control. **Round 5 is next — the WinUAE studio**: `AmigaInstallPanel` off tab 1, `FirstBootPanel`'s rehearsal given a home, and `packages.folder` read-only for seed (spec § 5). **That last one is an open spec item, not a done one**: the whole-branch review (I3, 2026-09-10) found `session.packages.folder` is still *written*, where the four-tab spec § 5 says the session's copy is read-only and seeded. Measured on the fix wave's tree: `useBuildSession`'s `setPackagesShape` persists the whole `{folder, chosen}` shape on every tick change, and the folder-removal path writes `folder: null` outright when the folder leaves the material list (`src/lib/useBuildSession.ts` ~225-243, ~282-288). **The fix wave changed no code for it** — round 5 closes it when the Amiga-side panel moves. Findings list: `D:\Projeler\Amiga\ART-test-bulgulari-2026-09-07.md`.
