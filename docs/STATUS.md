...
| | |
|---|---|
| **Last updated** | 2026-08-24 (**Verify against a card is on the step that has the card** — wave 3, closing the OS Builder flow item: it is on `birimler` under the volumes panel, not on the install step and not on the card builder. Before that: **Verify against a card is its own file, and the tree carry got wider on the way out** — the first cut of `OsInstall.tsx`, 1 722 lines to 1 553, with 7 new tests and two vacuous assertions caught before the mutation round. Before that: **One card for the build, the way there is now one Kickstart** — ART-197's last duplicated field but the per-partition folders: the image ART writes and the image the volumes step prepares are one value. Before that: **The nine trees can be told apart without running anything** — ART-197 wave 2's first row: `chain::trees_in` lists the builds inside a folder with the release, the size, the component count and which packages already ran on the Amiga against each. Before that: **ART can name a disc, and the first thing that found was that it had been naming the wrong one** — [ART-229](ISSUES.md): a folder holding a real AmigaOS **3.1** Workbench disk was announced as the user's 3.2 folder, because `Fonts` and `Locale` carry no version suffix in either release. The table is the shipped recipes rather than a hash list. Before that: **The card builder has a test file, and the mutation round found two of its own assertions worth nothing** — six tests over `CardBuilder.tsx`, closing [ART-223](ISSUES.md)'s disclosed survivor; the pair that matters pins the panel's own claim that `SAFE_CREATE` is answered before the button, both arms. Six mutations after the rewrite, six fell. Before that: **The official 3.9 Locale update is a package now** — `locale-39`, the wholesale half its own installer copies whatever languages are chosen, so a tree's `Devs/Keymaps` holds **49** keymaps against the CD's 22 with `türkçe` among them. The per-language half is deliberately not built. Before that: **Seven megabytes that used to reach the volume packed and misnamed now arrive as files** — [ART-228](ISSUES.md): **3 263** `.Z` files per 3.2 tree, the whole help system among them, expanded by an LZW reader written for it and checked against 7-Zip byte for byte. V40 19 839 113 bytes against 12 751 572 before. Five mutations, all five fell; two limits disclosed rather than glossed. Before that: **The 3.2 keyboards are in, and behind them [ART-228](ISSUES.md)** — **3 263 files** in every AmigaOS 3.2 tree ART builds are `compress`-format `.Z` under the wrong name, where the release's own Installer writes them decompressed: the whole help system in every language, plus the Turkish fonts and catalogs. Filed, not fixed. The keymaps half is done and verified against both of the owner's ROMs, and it re-measured [ART-224](ISSUES.md) on real material along the way — 15 934 bytes of GlowIcons arriving in a tree rather than in a comparison of two ADFs. Before that: **A BoingBag install finishes now** — [ART-227](ISSUES.md), work-list item 1, measured then fixed. BoingBag 2's `Updater` was leaving its **321 768**-byte ROM update beside the CD's 127 956-byte one under a name `SetPatch` does not read, and BoingBag 1 was leaving ten files without the protection bits `Resident` needs. Both confirmed on the owner's own material before anything was written. The fix runs **on the host** against the staged copy — which is why it took a day rather than three: no quoting problem and no fifth `RunOutcome`. Verified end to end, and seven mutations all fell. Before that: **All the keyboards are in the distribution, and the Turkish one arrived without anybody typing its name** — [ART-226](ISSUES.md)'s first question built as a tick-box `keymaps` component, one `Subtree` rule so the medium carries the spelling: *keymaps: 44 entries, non-ASCII: ["türkçe", "türkçe.info"]*, 0 refusals, 1973 files in 16.28 s. [ART-227](ISSUES.md) filed: the BoingBag fix-ups HstWB does after the `Updater` — including moving `Devs/AmigaOS ROM Update.BB39-2` over the old file — and ART does not. Before that: **The Turkish tree renders Turkish** — the owner read `ç ü ş Ğ` in its Workbench menus, which closes the claim [ART-159](ISSUES.md) opened and withheld. **The next thing it cannot do is type them**: [ART-226](ISSUES.md), filed not fixed. `Devs/Keymaps` is empty in every tree ART builds — empty on the disc too, so not a copying defect — and the Turkish keymap is not on the CD at all. It is in `Locale3_9.lha`, which the owner already has, along with 15 Turkish font families against the CD's 13. Before that, on 2026-08-23: **The Turkish tree was booted, the fonts were looked at, and they were not there** — [ART-225](ISSUES.md), found in the first minute of the first boot and fixed the same evening. Thirteen ISO-8859-9 font descriptors were placed as `X.FONT`; `diskfont.library` matches that suffix case-sensitively, so the system could see none of them while every file sat on the volume and every test was green. Settled by a one-variable experiment on the running machine — one suffix lowered, that one font appeared, the other twelve did not — and fixed by writing all 26 destinations in the disc's own spelling, which the real-media hook now asks the disc to confirm (*all 26 names are the disc's own spelling*). Re-measured: 0 refusals, **1929 files / 194 directories / 19 016 030 bytes in 16.83 s**. Before that, the same day: **[ART-159](ISSUES.md)'s hazard 2 is closed by shipping it, and the guard written to hold it found two live defects in the recipe next door.** The owner scoped both language trees in and both were built from their own `AmigaOS39.iso`: **0 refusals, 2339 items, 19 016 030 bytes, 1929 files and 194 directories in 16.30 s**, with **9 of 9** euro `.country` files matching the disc's euro copy rather than its base copy. The Turkish set is **13 font families, 13 `.FONT` descriptors, 37 size files** — and three of the entry's own recorded facts were wrong before this round measured them: `Special-Locale`'s keymaps and printer drivers are the *Czech* branch, the font list was three short, and the release's own Installer names neither drawer. The first **non-ASCII `from` path in any shipped recipe** (`TÜRKÇE`) resolves, and the empty `refusals` is what proves both that and the zero collisions the no-override decision rests on. Writing `workbench-39`'s positional rule as a general test then failed the **AmigaOS 3.2** recipe on first run ([ART-224](ISSUES.md)): `glowicons` and `modules-a1200` declared an override over `storage` from *above* it, so both were inert — `glowicons` cost sixteen icons a user had ticked for, `modules-a1200` cost nothing because both disks' `C/LoadModule` is the same file to the byte, and both are stated. The 3.9 screen's five rows all read `AmigaOS3.9`; a recipe can now name its own row. Six mutations against the recipe and catalogue guards, all six fell; two against the real-material hook fell on an earlier assertion than the one aimed at, so that hook's final byte comparison is **not** mutation-proven and says so. Before that, the same day: **The debt register went from thirteen open entries to six, and the two that closed hardest closed by measurement.** [ART-172](ISSUES.md): the language pack really does land on the base `Locale` — **29 collision rows** against the round that reported `rows=0`, 12 upgrades, 13 same-version, 2 downgrades, 2 unversioned, every one `declared`. And the reason two shipped documents gave for it was **wrong**: they said the disc spells the drawer `TÜRKÇE` and the pack `türkçe`, so `fold_amiga_case`'s Latin-1 range is what brings them together. Building `locale-base` from the owner's own disc places `türkçe`, lower case — the same spelling — and the whole measurement re-run with the fold cut back to ASCII gives the identical 29 rows. The range is still right and three tests hold it; it is not this package's reason. [ART-171](ISSUES.md) closed the same way and its test's **own second assertion was the thing refuted**: it demanded a `.uaem` sidecar beside every package-placed file, and ART's rule is the opposite — an archive states no AmigaDOS metadata, so §89 forbids inventing one. 36 files into a drawer that was not there, 36 manifest rows, no invented metadata. [ART-179](ISSUES.md) was going to delete 28 dead catalogue keys and **ten of them were not dead**: `OsBuilder.tsx` renders them from keys the Rust-side distro registry names, which the scan could not see — they had sat three days under a reason that was untrue. The scan now reads that file, so they are held by their reader. [ART-184](ISSUES.md)'s other half is built: a crash-leftover sweep that fails **closed** — no instance lock, no sweep — and it made two shipped sentences untrue (*"ART never deletes it for you"*), both rewritten in the same commit with a test pinning the promised age to the constant. Also closed: [ART-187](ISSUES.md), [ART-183](ISSUES.md), [ART-060](ISSUES.md). [ART-159](ISSUES.md)'s hazard 2 was **measured but not closed** that day — `Special-Locale` on the owner's disc is 245 nodes in three languages, and the Turkish one is the **ISO-8859-9 font set** — and it is closed by the round above, which also corrected three of the facts recorded here. Before that, on 2026-08-22 (**The engine builds a distribution from the owner's own material; the screen still has not.** A real AmigaOS 3.9 tree — 1242 files, 22.79 s — came out of the engine's own hook, while the owner driving the OS Builder could not produce one at all. Nine entries closed today, six of them found by them driving it, and [ART-204](ISSUES.md) — `config.txt` is a conditional-section format and the merge was flat, keeping one `kernel=` line out of four — is the likeliest reason no card ART built has ever booted. Everything is merged to `main` and pushed. **First action next session: drive the OS Builder and build a tree from it.** Before that: **The OS Builder is a sequence of steps, and the folder ART builds is carried to the steps that use it** — wave 1 of the flow design, on `osbuilder-wave-1` and **not merged**, because Task 7 was not done: the owner had ART open, `cargo` could not replace a locked `.exe`, and **no step has been seen on a screen**. [ART-197](ISSUES.md) and [ART-198](ISSUES.md) are closed with tests, and every named mutation was run and fell. The design's own key count was wrong — it says 22 remembered keys, there are 36 — so the migration is per key rather than per list: eleven taken over and seeded, twenty-five left where they are. See the session log for the rest. Before that, on 2026-08-21: **Both of the owner's BoingBags installed, and the booted tree says so.** [ART-193](ISSUES.md) is fixed and it was one line: ART's generated `Startup-Sequence` had never run the tree's own `C:AddDataTypes REFRESH QUIET`, so every install ran against a `datatypes.library` with an **empty** descriptor list — a state a booted AmigaOS 3.9 is never in — and the `Updater` hung for ever inside its own interface, opening nothing, printing nothing and writing nothing. Found by bisection, not by reasoning: a full boot environment installed, then assigns-only hung, `LoadWB` alone hung, `IPrefs`+`ConClip`+`LoadWB` hung, and **`AddDataTypes` alone installed**, byte-identically. Stack (4 096 → 65 536, confirmed on the machine) and the mounted CD were tested and are negatives with evidence. Through ART's own `compose` → `install` path on the owner's real material: `BoingBag39-1 (1).lha` in **169.1 s** (`Succeeded`, `Promoted`, 3 795 → 3 859 files), then `BoingBag39-2.lha` on that result in **138.1 s**. **Asked, not inferred**, the tree then answers `Workbench 45.3 (07-Dec-01)`, `version.library 45.3`, `workbench.library 45.127` where it used to answer `Workbench 45.1 (13-Nov-00)` — the round's own bar, met. `PROVISIONAL_DEADLINE` drops from a 60-minute guess to **30 minutes**, ten times the longest measured successful install. Both refusal paths still refuse and copy nothing, and the disc refusal is now true by measurement: without the disc the `Updater` reaches its own *"Checking AmigaOS 3.9 CD-ROM…"* line and stops on an AmigaDOS `Please insert volume AmigaOS3.9` requester nobody can answer. **Nothing decrypts anything and no password is bypassed** — the package's own `Updater` opens its own payload, inside the emulator.)) |
...
| **i18n** | `en.json` and `tr.json`, 1834 leaf keys each (1847 until ART-179 removed seventeen keys nothing rendered), parity enforced by `pnpm test` |
...
*Last touched 2026-08-23, second session of the day.*

**[ART-159](ISSUES.md) is closed and [ART-224](ISSUES.md) was found and fixed
closing it.** The open register is **five**: ART-166, ART-118, ART-062,
...
1. **A card flashed and an A500 booted.** Not code. The owner does this part.
2. **[ART-226](ISSUES.md) — the keyboard.** The glyph question is **closed**:
   the owner read `ç ü ş Ğ` in the Workbench menus of the fixed tree and said
   so. Trying to *type* them found the next thing: `Devs/Keymaps` is empty in
   every tree ART builds, so only the ROM's `usa` is available — for every
   language, not just Turkish — and the Turkish keymap is not on the CD at
   all. It is in `Locale3_9.lha`, which the owner already has (1 008 bytes,
   plus a country file, a newer `.language`, the flags and **15** Turkish font
   families against the CD's 13). Three scoping questions in that entry, all
   the owner's.
3. **The package-bundles work list**, items 8 (`.vhd`), 9 (the 3.2.2.1/3.2.3
   recipes), 10 (the package catalogue as data) and the last two lines of 11.
4. **`CardBuilder.test.tsx`** — that panel still has no test file at all, a
   disclosed survivor from ART-223.

**The pattern this round should be read for.** The two defects that mattered
were not in the thing being built. `every_override_is_declared_after_what_it_overrides`
was written to generalise a rule the 3.9 recipe already obeyed, and it failed
the **3.2** recipe the first time it ran — sixteen icons a user had ticked for,
silently reverted for months. Writing the general form of a rule you already
follow is how you find out who else does not.


*This block's own leading sentence was superseded on 2026-08-22 and is kept
below for the history. What follows first is where the work actually stands at
the end of that day.*

**The screen builds distributions, and it downloads packages. Both were driven
by a person and both worked.** The tree: 1146 files, 126 directories,
6 320 749 bytes from 15 ADFs into `E:\amiga\Amigatolon\os39\art5`, asked rather
than inferred (`distribution.json` says `AmigaOS 3.2` while the folder is named
`os39`). The packages: 43 downloaded from live Aminet, 10 refused by name
before the tick, 0 failed, all six outcomes recorded separately.

**What is still owed, in order:**

1. **A card flashed and an A500 booted.** Unchanged, and still not code. SD-1
   names no missing feature; the bar for 1.0 is hardware, and the owner has
   said they will do that part themselves.
2. **The package-bundles work list's items 4-9** — identify install media by
   content rather than volume name, `.vhd` output, the 3.2.2.1/3.2.3 recipes.
   Both of the entries that stood in front of them are now **closed**:
   [ART-205](ISSUES.md) (the plan and the tree now agree exactly on both of
   the owner's own releases) and [ART-196](ISSUES.md) (ART stages where the
   user says, and refuses rather than falling back to `C:`). What is left of
   the scratch problem is [ART-184](ISSUES.md)'s other half — a sweep for
   what a crash leaves behind.
3. **Nothing about SFS.** Decided and recorded: see the work list's "Not on
   this list, and why", and the deferral at the head of
   `ART-brief-sfs-integration.md`.

**The pattern this day should be read for.** Seven defects were found by the
owner driving screens — ART-207 through ART-214 — and not one was reachable by
a test that already existed. Two more came from asking a reviewer about a
*named risk* rather than for a general opinion, and one of those was the
Critical. Against that, a plan's own named mutation turned out inert because
its fixture was already in the state the code produced. Tests are right about
code and wrong about the world; driving the thing is what finds the rest.

---

**The engine builds a distribution. The screen still has not.** *(Superseded
2026-08-22 — both halves are now done; kept for the history.)* That sentence
is the whole state of this round and everything below serves it.

On 2026-08-22 the install engine was run against the owner's own
`AmigaOS39.iso` and produced a real tree — **1242 files, 105 directories,
14.9 MB, in 22.79 s**, `workbench-base` 450 + `workbench-39` 792, manifest
written, into `E:\amiga\Amigatolon\os39\art1`. **That came from the
engine's own `#[ignore]`d hook, not from the OS Builder.** The owner drove the
screen the same evening and could not produce a distribution at all. Do not
read the tree as proof the screen works.

**First action tomorrow: drive the OS Builder and build a tree from it.**
Everything needed is now in place — [ART-203](ISSUES.md) removed the dead end
where every folder the picker could return was refused, and
[ART-197](ISSUES.md) carries the result to the steps that use it. Neither has
been seen working by a person. If it works, say so here with the numbers; if it
...
folder was the one holding their AmigaOS 3.9 disc, kept from the previous
session because the media folder was remembered once for all releases
([ART-207](ISSUES.md), fixed) — and sixteen true sentences were the only thing
the screen had to say about it ([ART-208](ISSUES.md), fixed). **Neither of
those is the tree.** The next action is unchanged and now has two fewer things
in its way: point the media folder at the 3.2 ADFs
(`E:\amiga\Amigatolon\paketler\3.2\AmigaOs 3.2\ADF` — 35 disks, all sixteen
volume names the recipe wants present and unique, checked) and build.

### What closed on 2026-08-22, and what it cost to find

Nine entries: [ART-197](ISSUES.md), [ART-198](ISSUES.md),
[ART-199](ISSUES.md), [ART-200](ISSUES.md), [ART-201](ISSUES.md),
[ART-202](ISSUES.md), [ART-203](ISSUES.md), [ART-204](ISSUES.md),
[ART-206](ISSUES.md). Six of them were found by the owner driving the screen,
and the measurement they share is **seven identical failed runs** in
`operations.jsonl` — the same request, unchanged, seven times, because the
refusal rendered 209 lines above the button that caused it.

**Two are worth remembering past this round.**

[ART-204](ISSUES.md) is the likeliest reason no card ART built has ever booted:
`config.txt` is a **conditional-section** format and the merge treated it as a
flat one, so a real Emu68 file came out with **one** `kernel=` line out of
four, in the stealth stanza, and three board stanzas with none. Measured, not
reasoned about. No test could have caught it, because every `config.txt`
fixture was a flat file ART had written itself — a reader and a writer agreeing
with each other, the shape that already cost ART-032..035 and ART-079.

[ART-203](ISSUES.md) is the reason no tree had ever been built from the screen:
`apply` refused every folder that existed and a folder picker can only return
one that does. `SAFE_CREATE` now means what it is for — never over somebody's
data — and an empty directory holds none.

**And one correction the owner made and was right about.** ART-202's first fix
rendered the refusal in two places; they read it as two errors — *"aynı uyarı
tek ekranda 2 tane"*. With ART-201 in place a refusal leaves no preview card,
so the panel is short and both boxes land within a screen of each other. One
box, at the control.

### The research, and the decision that came out of it

The owner asked, mid-round, whether ART should be rebuilding what the **Emu68
Imager** already does — *"bu şekilde karmaşık ve başarısız bir installer ile
ilerlemeyelim"* — and then answered it: *"bizimkisi de çalışmalı"* and *"ART'ın
amacı bir suit yapmak Amiga retro meraklılarına"*. **ART keeps building cards
and the standard is that they work.** The incumbent is not a competitor to
reason about; it is the reference implementation to measure against.

Four sources were read and quoted rather than recalled:
[the research](superpowers/specs/2026-08-22-emu68-imager-research.md),
[the argument](superpowers/specs/2026-08-22-should-art-build-cards.md),
[the work list](superpowers/specs/2026-08-22-work-list.md) (12 items, ordered,
each with its evidence) and
[the package catalogue](amiga-package-catalogue.md) (47 Amiga packages with
their real sources; 41 are Aminet paths, which is the shape `core/sources`
already takes).

**It confirmed ART's card model** — MBR with `0x76` areas, one RDB each — from
an independent implementation, and showed the reference tool drives the same
`hst-imager` ART keeps as a fallback. **Two of the research's warnings are
worth repeating**: read `docs/superpowers/specs/2026-08-22-work-list.md` §"what
was NOT read" before treating the list as complete, and do **not** copy the
imager's `MaxTransfer`/buffer defaults over ART's — ART's came from measuring
the owner's own cards, which is the stronger evidence.

### After the screen is driven, the order is the work list

[ART-205](ISSUES.md) first — the plan predicts 17.6 MB where 14.9 MB lands,
**18% over and systematic**, because a file an overlay replaces is two plan
items and one file on disk. It is what a progress bar divides by. Do not
subtract the difference; decide what `total_bytes` is **for**.

Then [ART-196](ISSUES.md) (scratch on `C:`), then items 4–9 of the work list —
identify install media by content rather than volume name (the imager's own
table carries an `ADFSource` column, which is how it prefers Commodore and
Cloanto images over Escom's, whose language and keyboard files are missing),
`.vhd` output, and the 3.2.2.1/3.2.3 recipes whose media is spread across three
folders a one-folder scan cannot express.
...
**2290 Rust passed / 0 failed / 28 ignored · 837 frontend passed / 72 files ·
`pnpm lint`, `cargo fmt --check` and clippy `-D warnings` all clean.** The Rust total is unchanged because this wave touched no
Rust; the frontend total is 784 + 44, the tests this round added. i18n is
**1776 leaf keys** in each catalogue, up 9 — six step labels, the sentence a
step says when it is opened without a tree, and
`osinstall.result.carried`.

**Everything is on `main` and pushed.** Wave 1 merged with `--no-ff`, then
ART-204 and ART-203 as their own short-lived branches, each merged and deleted.
`main` is equal to `origin/main` and the working tree is clean.
