...
**The second debt round (`art-debt-2-0914`) is merged to `main` `--no-ff` as `23681a6` and pushed, both on the
owner's word.** The numbered items immediately below (down to the "ART-117 is fixed" entry) describe that branch
before the merge and are history now, superseded by the merge line in the Snapshot above — left as detail rather
than rewritten, because the reasoning and the counts in them are still correct for the day they were written.

**Third debt round, item 1: ART-321 is fixed on `art-debt-3-0915` (unmerged, not yet reviewed).** `mbr_slot_of`
used to name the wrong MBR slot when an earlier Amiga area was unreadable; `AmigaArea` now carries its own
`mbr_slot`, set when `read_card` builds it, so the slot survives an earlier area being skipped. Detail in
[ISSUES.md](ISSUES.md#fixed) under ART-321 and in the [session log](session-log.md).

**Third debt round, item 2: ART-317's survivors (b) and (c) are guarded on `art-debt-3-0915` (unmerged, not yet
reviewed).** A new `core::independence::only_clock_and_local_time_name_amiga_from_wall_or_system_now_unix` closes (b);
a new `core::preload::native::open_pfs3_writer` helper plus
`core::independence::libpfs3_writer_open_is_named_only_by_the_pfs3_writer_helper` closes (c). Detail in
[ISSUES.md](ISSUES.md#fixed) under ART-317 and in the [session log](session-log.md).

**Third debt round, item 3: ART-319's disclosed gaps are closed on `art-debt-3-0915` (unmerged, not yet
reviewed).** libpfs3 is `0.1.3+art.6`. `overwrite_file_in` is copy-on-write: the new content goes only into free
blocks, and the head anode, the freed old blocks and anodes and the entry's size all land in one commit. The
limit is that the whole new content needs free space of its own, or the call is refused `DiskFull` before a write.
`rename_in` over an existing destination is one commit, with the destination sent to the deldir exactly as
`delete_in` sends it. Every public mutator now has discard and lock tests of its own. This item's own throwaway
experiment found [ART-322](ISSUES.md#fixed), a case-only PFS3 rename that deleted the file and reported success —
closed by item 3b, below. Detail in [ISSUES.md](ISSUES.md#fixed) under ART-319 and in
`.superpowers/sdd/2026-09-15-debt-3-round/item3-report.md`.

**Third debt round, item 3b: ART-322 is fixed on `art-debt-3-0915` (unmerged, not yet reviewed).** libpfs3 is
`0.1.3+art.7`. `rename_in`'s destination lookup used `name_eq_ci`, so a case-only rename in the same directory
(`rename_in(root, "File", root, "FILE")`) found the source itself as "an existing destination" and deleted it,
reporting `Ok`; the identical name, same case, hit the same path and was worse. `rename_in_impl` now checks
whether a found destination is the source entry itself — same parent anode, same anode number — before treating
it as one to delete: when it is, an identical name is a no-op success and any other case-only change is rewritten
in place (`rename_dir_entry_in_place`), keeping the entry's anode, size, protection, dates and comment; a
same-directory destination that differs only in case from a genuinely different entry is still replaced, as
before. pfs3aio's own `RenameAndMove` (`directory.c:2064-2076,2130`, `tonioni/pfs3aio` `211f7f0`) was read this
time and confirms the direction — it never deletes when the found destination is the source. Detail in
[ISSUES.md](ISSUES.md#fixed) under ART-322 and in `.superpowers/sdd/2026-09-15-debt-3-round/item3b-report.md`.

**Third debt round, item 4: ART-117's M5 disclosure is closed on `art-debt-3-0915` (unmerged, not yet reviewed) —
ART's three `$VER:` readers are one.** `core::amigaver::read_loose` is the unification; `rdb::version_from_ver_string`
and `rdbedit::program_name_from_ver_string` are now thin calls into it. `amigaver::read` (the strict reader) is
untouched, kept as a separate accessor exactly because its control-byte rejection is still what
`core::osinstall::collide`, `core::amigainstall::packagevol` and `core::osinstall::apply` rely on — not, as M5 used
to say, on a `core::gameindex` dependency that a grep of every caller did not find; corrected in place in
[ISSUES.md](ISSUES.md#fixed). Detail there under ART-117 M5 and in
`.superpowers/sdd/2026-09-15-debt-3-round/item4-report.md`.
**Next:** the third debt round's item 6 — final review, then merging and the 0.9.4 build are the owner's word. None of the items below are it.

0000000000000000000. **ART-117's final whole-branch review is fixed on `art-debt-2-0914` (not merged).** Review
`.superpowers/sdd/2026-09-14-art-117-rdb-embed/final-review.md` ("With fixes"), report `fix-wave-report.md` beside
it, detail in [ISSUES.md](ISSUES.md#fixed) under ART-117. I1: a file or card driver whose `$VER:` names no program
is `DIFFERENT-DRIVER`, not let through to the version comparison. I2: a written, verified edit whose journal file
cannot be removed ends as `ART-RDB-EDIT-JOURNAL-LEFT` — the edit stands, delete the file, do not undo it; a failed
last sync still rolls back. I3: `core::preload::run` and `run_with_fallback` return a stop carrying the outcome, the
log record keeps the Driver and RDB-backup lines beside the failure, and the result panel says the run stopped and
the RDB change stays. M1/M2: appends before replaces, every RDB edit before any format. M3 corrected in the spec
(14 replaces on zero blocks, not 46; no safety gap), M11 in `architecture.md`, M12 filed as [ART-321](ISSUES.md#open).
Counts: `cargo test --lib` `3411 passed; 0 failed; 60 ignored` twice; `pnpm test` 111 / 1685; fmt, clippy, deny,
lint and four sweeps clean. **Scoped re-review fix, 2026-09-15:** that I2 journal used to be byte-identical to a
crash's, so the next run said "undo it first" and the File Manager would have undone the verified edit. The RDB edit
now writes a finished mark into the journal header (version field → `"DONE"`, synced) before removing it, and rolls
back if the mark will not write. A marked journal is refused by `open_rdb` as `ART-RDB-EDIT-JOURNAL-FINISHED`,
never rolled back by recovery, and offered in the File Manager for deletion only; an unmarked one is `JOURNAL-PENDING`
as before, and volume-writer journals are unchanged. An older build refuses a marked journal as an unknown version.
A mark that fails and cannot be taken out either now ends `JOURNAL-LEFT` (the edit stands, delete the journal), not
"undo it"; the File Manager's finished-journal card has a jsdom test (`FileManagerRecovery.test.tsx`). Six mutations
red, none survived. Counts: `cargo test --lib` `test result: ok. 3421 passed; 0 failed; 60 ignored; 0 measured; 0 filtered out` twice (443 s, 412 s, `TMP`/`TEMP` on `E:`); `pnpm test` `112` / `1687`; fmt, clippy, lint and three sweeps clean. **Next:** push and watch CI once, then the owner's merge word.
...
00000000000000000. **ART-117 is fixed — `art-debt-2-0914` merged to `main` `--no-ff` as `23681a6` and pushed, both on the owner's word (2026-09-15) — ART embeds or replaces a filesystem
...
`docs/superpowers/plans/2026-09-14-art-117-rdb-embed.md`, 12 tasks,
`.superpowers/sdd/2026-09-14-art-117-rdb-embed/`. **What shipped, one sentence each:** the walk
(`core/rdbedit.rs::walk_strict` refuses any block it cannot account for block by block, strict enough to have found
a stale RDB copy and old PFS3 structures already in the owner's card's reserved range that nothing had noticed);
allocation and the raise (`allocate` places a new driver above everything live and only raises `RDBBlocksHi` over
zero blocks, never past the partitionable area, decision 12); the backup (the RDB area is copied to a file the user
chooses before a byte is touched, `atomic_create_new` so an existing file is never overwritten, read back to
confirm); the four stages (`core/preload/embed.rs::write_journalled` — Data, Header, HighRdsk, Link — synced and
journalled, the one-sector link last so the driver becomes visible to a walk at exactly one instant); verification
(every written block is read back and re-walked before the journal commits; a mismatch rolls back byte for byte);
the screen (`VolumePreload.tsx` asks for the backup path, blocks Run without one when the plan edits the RDB, shows
the replace preview with both versions and the blocks it touches, and renders every `PlanNote`). Twelve tasks, each
with a red test before the code and mutation testing after (every mutation the brief named was seen to fail its
test, some with a wider blast radius than predicted — disclosed in each task's report, not silently narrowed).
**The Step 1 counts, twice:** `cd src-tauri && cargo test --lib` → `test result: ok. 3398 passed; 0 failed;
60 ignored; 0 measured; 0 filtered out` (both runs identical, `TMP`/`TEMP` on `E:`; fmt and clippy clean); `pnpm
test` → `Test Files 111 passed (111)`, `Tests 1683 passed (1683)`, `pnpm lint` clean; `control-byte-sweep.py`,
`scratch-root-sweep.py`, `scratch-guard-sweep.py` and `contrast-check.py --quiet` all clean; `oracle-check.py`
...
this branch); `31566bf` bumped `rustls` to 0.23.45 for RUSTSEC-2026-0285, and `cargo deny check` then gave
`advisories ok, bans ok, licenses ok, sources ok`. **Owed by the owner:** the byte copy and the
ignored hook are already done (Task 11, checked against `hst.imager` and amitools' `rdbtool`, both agreeing) —
what remains is mounting both PDS partitions of the edited copy in WinUAE or on a PiStorm and asking the loaded
handler for `version full`. **Next:** merging is the owner's word.

0000000000000000. **ART-062's mechanical part is done — merged to `main` as `23681a6` with the second debt round (2026-09-15) — the one Turkish string
a plain browser could measure was clipped, fixed, and the entry's own table was corrected in place. ART-062
stays open.** Brief, not a spec — `.superpowers/sdd/2026-09-14-debt-2-round/art062-brief.md`, design approved
in chat by the owner. `scripts/tr-overflow-check.py` (new, modelled on `zoom-check.py`/ART-099) drove headless
Chrome against `pnpm dev`: all 16 top-level routes, 1280x800/1024x768/960x768 (the shell's own `minWidth`),
Turkish against an English control in the same mounted tree, 48 (route x width) runs, 0 crashes. One real
Turkish-only hit: `files.pane.nothingOpen` ("Nothing open" → "Hiçbir şey açık değil", +83%) clipped 10px in
`.tc-path-text`, 1024x768 only (not 1280, not 960 — non-monotonic, not investigated further). Shortened to
**"Açık bir şey yok"**, the owner's choice; `en.json` unchanged; no test pinned the old string. Re-run after the
fix: 48 runs, 0 errors, 0 crashes, Turkish-only list empty at every width. The same pass found the entry's own
table had decayed — `pistorm.saveSync` and `pistorm.profile.classic.badge` no longer exist, "Grid" belongs to
Collection Studio, not FileManager — corrected in place in [ISSUES.md](ISSUES.md#open) rather than silently
worked around. `pnpm lint` clean; `pnpm test` `110`/`1671` (unchanged); `control-byte-sweep.py` and
`contrast-check.py --quiet` clean; no Rust file touched. **Still open, for a person:** reading the Turkish
catalogue itself (2262 keys), and `hardDisk.bootablePri` / job status "Done"/"Failed", both gated behind a
live Tauri session a plain browser cannot fake. **Next:** merging is the owner's word; ART-062 itself needs a
person reading Turkish on screen, which this round could not do.

000000000000000. **ART-250 is fixed — merged to `main` as `23681a6` with the second debt round (2026-09-15): ToolTypes are Latin-1, byte-exact.**
Brief, not a spec — `.superpowers/sdd/2026-09-14-debt-2-round/art250-brief.md`, design approved in chat by the
owner. `tooltypes()` decoded with `String::from_utf8_lossy` and `set_tooltypes()` encoded with `tt.as_bytes()`
(UTF-8); real AmigaDOS ToolTypes text is ISO-8859-1, so a NewIcon `IM1=`/`IM2=` pixel-encoding byte above 0x7F
(the format runs the full 0x20-0xFF, one byte each) came back as `U+FFFD` and re-encoding it did not reproduce
the original bytes — measured on 69 of the owner's 798 real icons. Both ends now decode/encode through a private
`latin1_decode`/`latin1_encode` pair in `core/amigaicon/mod.rs` — the identity cast on code points 0..=255
`core/adf/bcpl.rs` already uses (ART-074) and `core/osinstall/apply.rs` reimplements independently for
`S/User-Startup` text, kept as a third private copy rather than imported, since each field's encode behaviour on
an out-of-range character differs. `set_tooltypes` refuses a character above `U+00FF` by name (index and text)
rather than substituting `?`, safe because it has no production caller today. Three new tests
(`a_newicon_tooltype_with_high_bytes_round_trips_byte_identical`, `a_latin1_high_byte_decodes_to_its_character`,
`set_tooltypes_refuses_a_character_with_no_latin1_byte`), each red before the fix and green after; three
mutations, each red and restored (never `git checkout --`). `cd src-tauri && cargo test --lib`:
`test result: ok. 3326 passed; 0 failed; 59 ignored` twice; fmt and clippy clean; `control-byte-sweep.py` clean.
The `#[ignore]`d oracle test and `scripts/icon-oracle-check.py` both re-run against the owner's
`E:\amiga\Amigatolon\os39` before and after: `checked=798 failed=0 no_drawer_data2=0 lossy_tooltypes=69` before
(matching the prior measurement exactly), `checked=798 failed=0 no_drawer_data2=0` after, the bucket gone. No
`CHANGELOG.md` entry — no production code calls `set_tooltypes` today, so nothing user-visible changed. **Next:**
merging is the owner's word.

00000000000000. **ART-319 is fixed — `art-319-writer-rollback` merged to `main` `--no-ff` as `f916301` and pushed, both on the owner's word (2026-09-14): the PFS3 writer returns to its
last commit on error, and a commit that fails part-way locks it.** Brief, not a spec —
`.superpowers/sdd/2026-09-14-art-319-writer-rollback/brief.md`, design approved in chat by the owner. Every
public `Writer` mutator runs through a new `guarded` wrapper (`vendor/libpfs3/src/writer.rs`): on error it
discards back to the last successful commit through a new `Volume::reload` (`vendor/libpfs3/src/volume.rs`,
`from_device`'s own parse shared rather than duplicated) — `pending_writes` cleared, the rootblock/extension/
anodes/bitmap/cache rebuilt from the device, the reserved and data bitmaps reloaded. If the failure came from
inside the commit itself (the rootblock extension write, `flush_pending`, the rootblock cluster write, or
`set_volume_name`'s own direct write), the device may already be half-written (M5: pending writes land in
place, not copy-on-write), so the writer locks instead — a new `Error::CommitFailed`, mapped to
`CoreError::Pfs3WriterLocked` (`ART-PFS3-WRITER-LOCKED`), a sentence that says to reopen and check the volume.
libpfs3 is `0.1.3+art.5`. Three tests, each seen red on the unfixed writer before the fix landed:
`a_failed_pfs3_write_discards_its_partial_allocation`, `a_failed_pfs3_delete_stages_no_deldir_entry`,
`a_pfs3_commit_failing_part_way_locks_the_writer` (plus a fourth pinning the error's sentence). Four mutations,
each seen red and restored green; ISSUES has the full detail, red lines, and disclosed out-of-scope cases
(`overwrite_file_in`, unreachable from ART today with its evidence; `rename_in`, unreachable — ART never
renames on PFS3). `cd src-tauri && cargo test --lib`: `test result: ok. 3321 passed; 0 failed; 59 ignored` twice
(`TMP`/`TEMP` on `E:`); fmt, clippy and `cargo deny check` clean; control-byte, scratch-root and scratch-guard
sweeps clean. **Final review** (opus, `eeb3d62..0c4287a`): *With fixes*, 0 Critical, 2 Important (the lock
sentence was claimed as something a user sees — the failing call returns its own error and ART never calls again;
`set_volume_name`'s lock untested), 5 Minor; all fixed in `9e08811`/`11579cf` (two more lock tests, one of them
for a reload that cannot read the device; one truthful lock sentence) and re-reviewed clean. Suite after the fix
wave: `test result: ok. 3323 passed; 0 failed; 59 ignored` twice, and once more on the merged `main`. **Next:** a
release is the owner's word. The ART-317 bullet below said "Next: ART-319 if the owner
schedules it" — it did; this entry is that pickup, not a dangling pointer any more.

0000000000000. **ART-317 is fixed — `art-debt-0914` merged to `main` `--no-ff` as `7725afa` and pushed, both on the owner's word (2026-09-14), with the debt round's other four plans: every Amiga date ART makes from an instant is
now local wall time, as AmigaDOS stamps it.** Design first (`docs/superpowers/specs/2026-09-14-art-317-local-time-design.md`):
the offset comes from `chrono::Local`, called only in `tools/local_time.rs`; the seam is a trait,
`core::clock::AmigaClock`, passed explicitly (no process-wide setter — Cargo runs tests on parallel threads);
libpfs3 is `0.1.3+art.4` (`FormatOptions::datestamp`, `Writer::set_entry_date`, `ART-PATCH.md` records both).
Nine tasks: the trait itself; the Windows-backed `tools::local_time::LOCAL_TIME`; ART's FFS/OFS writer; a new
ADF's stamps; a copy source's host mtime (a `.uaem` date passes through untouched — already wall time); libpfs3
and `NativeFormatter`'s PFS3/FFS format and copy-in; a listed date read back and a disc's `.uaem` sidecar; an
install disc's own recording date (`SCAN_CACHE_SCHEMA` 2 → 3, so a pre-fix listing misses and every install disc
is read again once); and the gate — `VolumeWriter::open` and `create_blank_adf` became `#[cfg(test)]`, so the
compiler refuses them in product code; `AMIGA_EPOCH_UNIX` arithmetic outside `clock.rs`/`adf/bcpl.rs` fails
`core::independence::core_makes_amiga_dates_only_through_the_clock`; and a new guard,
`core::independence::commands_and_tools_never_name_utc_clock_outside_a_test` (fix wave, 2026-09-14), refuses
`UtcClock` named in product `commands/`/`tools/` code outside a test. `architecture.md`'s trait table gained a sixth instance. **Suite of record:** `cd src-tauri && cargo
test --lib` `test result: ok. 3317 passed; 0 failed; 59 ignored` twice after the final review's fix wave
(`90d7bc2`, `a3b879d` — the `UtcClock` guard is the one new test; 3316 twice before it);
fmt, clippy and `cargo deny check` clean; Vitest 110 / 1671, `pnpm lint` clean (this plan changed no frontend
file); control-byte, scratch-root, scratch-guard and contrast sweeps clean. **Oracle:** `scripts/pfs3-oracle-check.py`
exit 0 — ART and hst-imager agree, both directions, names, sizes, bytes and protection bits. **Disclosed
survivors (final review I1, closed and open):** on a UTC CI runner, `offset_at` returning 0 passes every
non-ignored test — a weak guard on such a machine; the ignored `agrees_with_dotnet_timezoneinfo_across_seasons_and_years`
catches it on a non-UTC one. The fix wave (2026-09-14) added a round trip through the real clock to that same
test and ran it on the owner's machine: `test result: ok. 1 passed; 0 failed; 0 ignored; 0 measured; 3375 filtered
out` — offset and round trip both agree with .NET at all four instants. **Closed** in the fix
wave: a `commands/`/`tools/` site passing `&crate::core::clock::UtcClock` in place of
`&crate::tools::local_time::LOCAL_TIME` is now refused by
`core::independence::commands_and_tools_never_name_utc_clock_outside_a_test`; grepping `commands/**` and
`tools/**` for `UtcClock` found no site at all, product or test, so nothing needed an allow-list entry. **Still
open, neither guarded nor disclosed until now:** (b) calling `amiga_from_wall(system_now_unix())` directly
bypasses the clock's own offset lookup — both are `pub` and `clock.rs` is on the
`core_makes_amiga_dates_only_through_the_clock` allow-list, so no guard sees it; (c) a libpfs3 `Writer` opened
without calling `set_entry_date` falls back to UTC (`vendor/libpfs3/src/writer.rs:132-133`) and the independence
guards do not read `vendor/`. **Still owed by the owner:** mount a volume ART wrote under real pfs3aio in
WinUAE, which has not been tried under this fix. **Plan 5's final review** (opus, `6dcae37..6e76234`): *With fixes* — 0 Critical, 2 Important (both records
claiming more than the code), 6 Minor; all but M5 fixed in `90d7bc2`/`a3b879d` and the scoped re-review found
...
00000000000. **ART-311, 313, 314, 315, 316 and 318 fixed on `art-debt-0914` (not merged) — the vendored
crate is `0.1.3+art.3`.** ART's PFS3 partitions are now formatted with pfs3aio's deldir, and a name over
106 bytes is refused by name (both the owner's decisions of 2026-09-14). The tests and mutations for each
are in its own Fixed entry in `docs/ISSUES.md`. **Suite of record:** `cd src-tauri && cargo test --lib`
`test result: ok. 3289 passed; 0 failed; 58 ignored; 0 measured; 0 filtered out` twice; fmt, clippy and
`cargo deny check` clean; `control-byte-sweep.py`, `scratch-root-sweep.py` and `scratch-guard-sweep.py`
clean. **Oracle:** `scripts/pfs3-oracle-check.py`, exit 0, against the deldir-enabled format (`ART_PFS3_DRIVER`
pointed at the pfs3aio driver beside `hst.imager.exe`) — both directions agree, names, sizes, bytes and
protection bits. **Next:** its final review's fixes are in and re-reviewed; the merge is the item above's. **ART-317 (plan 5) is now fixed too — see the item above.** **Still owed by
a person:** a file deleted or undeleted on a real Amiga from a PFS3 partition ART formatted; a patched volume
mounted under real pfs3aio in WinUAE.

0000000000. **ART-312 is fixed: `art-312-anode-reuse` merged to `main` `--no-ff` as `28f2830` and pushed, both on the owner's word (2026-09-13); 0.9.3 carries it.** `libpfs3`'s writer resolves the anode index through its own pending writes, so one operation that allocates twice gets two different anode numbers; the vendored crate is `0.1.3+art.2`. A test red on 0.1.3's writer at `D009/F00070`, two mutations killed, `cargo test --lib` `test result: ok. 3258 passed; 0 failed; 58 ignored; 0 measured; 0 filtered out` twice, clippy clean. Filling a PFS3 partition natively is now bound by ART-311's ~21 500-anode ceiling rather than by silent corruption. A package was built from the branch head (`dc3cdd6`) before the merge. **0.9.3 carries it, published 2026-09-14.** **Next:** done — see above; a PFS3 volume past MAXSMALLDISK mounted under real pfs3aio in WinUAE is still owed by a person.

000000000. **ART-310 is fixed: `art-310-libpfs3-format` merged to `main` `--no-ff` as `37c7538` and pushed, both on the owner's word (2026-09-13; tasks 1–3 and 5 on the CachyOS Linux machine, task 4 on the owner's Windows machine).**
    - **What:** `libpfs3` 0.1.3 is vendored and patched (`src-tauri/vendor/libpfs3`, `0.1.3+art.1`): the format writes the super index level past MAXSMALLDISK and reserves anodes 0–4 at every size. Research, design and plan are under `docs/superpowers/` (2026-09-11 note, 2026-09-13 spec and plan); the run log, reports and rulings are in `docs/superpowers/notes/2026-09-13-art-310-handoff/`.
    - **Evidence, in ART-310's Fixed entry:** three tests seen red on 0.1.3, four mutations killed, the hst-imager oracle at both sizes on Linux, CI run 34756185385 on `bfdedd6`, and the owner's Windows runs — `cargo test --lib` `3257 passed; 0 failed; 58 ignored` twice, fmt, clippy and deny clean.
    - `plan_card_image`'s "never `NativeFormatter` past MAXSMALLDISK" rule is narrowed: the **format** is right at every size, but filling through `libpfs3`'s writer is not yet safe. The final whole-branch review (2026-09-13, verdict *with fixes*, all documentation) found two writer defects reachable from `native.rs`: **ART-312 🔴** (one anode number handed out twice — silent corruption) and **ART-311** (at most ~21 500 anodes in either mode, because `alloc_anode` searches only 256 anode blocks). Both are filed and unscheduled; `plan_card_image` has no production caller yet.
    - **Not yet safe to fill a planned card natively: the writer is still 0.1.3's.** The Windows machine's own, earlier ART-310 run (2026-09-11/12, local branch `art-310-windows`, never pushed) found a writer defect that hands one anode number out twice in one operation and silently corrupts files (3 of 22 000), fixed there in `9c7c845`, and grew small-mode index blocks on demand (`de26e58`). Neither is on this branch; ART-312 and ART-311 record them. The owner chose on 2026-09-13 to finish this branch first and port that work afterwards.
    - **Next:** port `art-310-windows`'s writer fixes, ART-312 first, with a test that writes unique content across more than one anode block, seen red first. The final whole-branch review ran before the merge (verdict *with fixes*, all documentation, applied in `87b3ae8`). **Owed by a person:** a patched large volume mounted under real pfs3aio in WinUAE, which is also what the prepared upstream patch waits for.

00000000. **Round 1 of the one-button card work is done and merged to `main` as `803ec5f` (2026-09-11, `--no-ff`, on the owner's word) and pushed on the owner's word.** ART-309 (the RDB's last partition no longer ends past the area it lives in — the cylinder count rounds down instead of up, `6b2849f`) and ART-308 (a card image is sized in decimal gigabytes with the established 95 % margin, and nothing that sizes a card image multiplies the label by 2³⁰ any more, including the second-system split that was still doing it after the first fix — `089909c`..`6ec1ae1`) are both **Fixed**. *Corrected by the final review:* this used to say "never multiplied by 2³⁰ anywhere in ART". Two harmless sites remain, both explained in ART-308: `OsBuilder.tsx:295`, a round trip that `core/distro` divides back, and `osBuilder.ts:115` `minCardBytes`, a parameter the hint never renders; both entries have their tests, red lines and mutations in `docs/ISSUES.md`. Task 7 closed the round: all 13 of the mutation table's mutations killed — 12 outright, the 13th (M10, an empty file silently counting as zero PFS3 blocks) survived every existing test as a weak guard and was closed with a new test, `an_empty_file_still_takes_one_block`. `[profile.dev.package.libpfs3] opt-level = 3` in `src-tauri/Cargo.toml` cut the `core::card::sizing` module's four real-libpfs3 fill tests from 101.05 s to 11.21 s and the whole `cargo test --lib` from ~120 s to ~35 s, suite kept green, no test dropped or shrunk. **ART-310 is open; the owner chose its fix route on 2026-09-11 — patch `libpfs3` and vendor it into ART, offering the patch upstream** (research first, in `docs/superpowers/notes/2026-09-11-libpfs3-format-fix-research.md`; then a design and a plan). The finding: a controlled experiment during the round (task 4's fill tests could not write past `MAXSMALLDISK`) found `libpfs3` 0.1.3's own FORMAT is wrong — a two-level superindex where its own reader and writer expect three, and anodes 0–4 left unreserved at every size — not ART's estimate or writer; the three routes (patch/vendor `libpfs3`'s `format.rs`, repair the two structures in ART right after formatting, or format with hst-imager and fill with libpfs3) are all in the ISSUES entry with the experiment's evidence. **Next: ART-310's fix (route A), then round 2 (content)** — round 2 is `core/card/content.rs` classifying and measuring a source into a `ContentMeasure`, staging it into a directory, WHDLoad-from-material by `$VER:`, and Kickstart placement — its own plan, written once this round's report is read. **The final whole-branch review's fixes landed on the same branch.** A content partition's 25 % headroom now clamps at `PFS3_CEILING_BYTES` rather than being refused. That constant is derived in `core::card::sizing` as the largest partition `create_rdb_layout` builds from an MB value inside PFS3's normal mode: 104 014 MB, which builds 211 331 cylinders, 213 021 648 blocks. `plan_card_image` plans nothing past it. Before this, a floor of exactly `PFS3_MAX_BYTES` built 1 712 blocks past `PFS3_MAX_BLOCKS`, and a Work split on a 256 GB card could build a piece past it too. ART-310 now records that every Work partition the planner makes on the flow's card sizes is past MAXSMALLDISK, so **round 3 was not to send those volumes to `NativeFormatter` until ART-310 was fixed** — its format fix landed on 2026-09-13 (item 000000000), but the writer is still 0.1.3's, so filling them natively waits for the writer fixes that item names. It also records that the small-mode anode cap is `libpfs3`'s writer, not PFS3. `SizingRefusal::DoesNotFit` naming no partition is deferred to round 2 in the plan. Reports and the mutation table: `.superpowers/sdd/2026-09-11-one-button-card-round-1/`. **Building a package is the owner's word, not yet given.**
...
5. **What is left open on the debt-clearing wave's own list is not code, and
   ...
   [ART-117](ISSUES.md) was reopened and fixed on 2026-09-14 (`art-debt-2-0914`).
   [ART-250](ISSUES.md) is
