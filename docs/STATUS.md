...
| **Tests — Rust** | `cd src-tauri && cargo test --lib` — the **full** suite, no `--skip` — on the merged `main` (`13e6707`), 2026-09-07 night: `test result: ok. 3062 passed; 0 failed; 51 ignored` (41.79 s). The same count printed twice on `7825549` the same evening, after the owner installed the antivirus exclusions ([ART-261](ISSUES.md#fixed)); before them the 89 `artwork` tests killed the harness |
| **Tests — frontend** | `pnpm test` on the merged `main` (`13e6707`), 2026-09-07 night: `Test Files 87 passed (87)`, `Tests 1194 passed (1194)` |
...
0. **The Windows 11 look is merged to `main` as `13e6707`** (`--no-ff`, branch `art-win11-look` deleted, 2026-09-07 night, the owner's decision after trying the build). Nine commits: four for the round, four for the whole-branch review's ten findings, one of docs. Re-measured on the merged tree: `pnpm lint` clean, Vitest 87 / 1194, `contrast-check.py` 105 pairs, control-byte and scratch-root sweeps clean, `cargo fmt --check` and clippy clean, the full `cargo test --lib` `3062 passed; 0 failed; 51 ignored` (41.79 s). The installers the owner tried are the branch's 22:53 build in `E:\amiga\ProjeART\build\`; `main` itself has not been rebuilt since the merge (same sources, only the docs commit on top). **Unpushed.** Still owed from the design round, for a later branch: the Settings screen restructured into settings cards, and the `.infobar` block adopted by `Refusal.tsx` and the outcome cards. The round's brief, review and ledger are local-only under `.superpowers/sdd/2026-09-07-win11/`.
