...
| **`main` / `origin`** | `main` is at `9234368` — ART-278/279 merged `--no-ff` from `art-278-runaway` on 2026-09-10 on the owner's word, branch deleted, the merged tree re-run green (full `cargo test --lib` 3197, Vitest 91 / 1391) — and **not pushed** until the owner says so; `origin/main` is `2b5096e` (CI run on `866dc2c` green; `2b5096e` is one docs line). Before that, identical at `866dc2c` (ART-281 merged `--no-ff` from `art-281-scratch` as `9fffc52` and pushed 2026-09-10 on the owner's word, taking `ad61a7a`'s release docs with it; CI run 34464414055 on the merge and 34464498495 on the docs commit `866dc2c` both completed `success`, 2026-09-10). Before that `7ee91fa` was 0.9.1's merge, tag `v0.9.1` |
| **Tests — Rust** | `cd src-tauri && cargo test --lib` — the **full** suite — on `art-278-runaway`, 2026-09-10: `3197 passed; 0 failed; 53 ignored (run twice, 2026-09-10)`. On the 0.9.1 release head, 2026-09-09: `test result: ok. 3181 passed; 0 failed; 53 ignored` (run twice, ART-059). On `art-281-scratch` (merged as `9fffc52`; the merge adds no line the branch head lacked — the branch was cut at `ad61a7a` and `main` had not moved), 2026-09-10: `test result: ok. 3188 passed; 0 failed; 53 ignored; 0 measured; 0 filtered out` (run twice). Two of the difference from 3181 are ART-281's own guard tests; the rest came with what `main` gained between the release head and `ad61a7a`, where the branch was cut, and was not attributed here |
| **Tests — frontend** | `pnpm test` on `art-278-runaway`, 2026-09-10: `Test Files 91 passed (91)`, `Tests 1391 passed (1391)`. On the 0.9.1 release head, 2026-09-09: 91 / 1388 |
