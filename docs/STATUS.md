...
**Item 5 is half proven.** The action tags were checked against each tag's own `action.yml` and our inputs.
`ci.yml`'s first run after the push, CI run 34960932926 on `e060e76`, completed `success` (2026-09-15, 22 min; the Node 24 action majors ran on `windows-latest`); `release.yml` is proven
only by 0.9.4's release run.

**Next:** the 0.9.4 package, then the tag `v0.9.4`, its release run and publishing — each on the owner's word.
