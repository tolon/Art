...
| **i18n** | `en.json` and `tr.json`, 1825 leaf keys each, parity enforced by `pnpm test` |
...
2. **The package-bundles work list's items 4-9** — identify install media by
   ...
   [ART-196](ISSUES.md) (scratch on `C:`) is still open and still first.
   [ART-205](ISSUES.md) is **closed** — the plan and the tree now agree
   exactly on both of the owner's own releases.
