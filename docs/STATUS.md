...
`delete_in` sends it. Every public mutator now has discard and lock tests of its own. New, open: [ART-322](ISSUES.md#open), a
case-only PFS3 rename deletes the file and reports success — measured on the writer before and after this item,
unreachable from ART today. Detail in [ISSUES.md](ISSUES.md#fixed) under ART-319 and in
`.superpowers/sdd/2026-09-15-debt-3-round/item3-report.md`. **Next:** the third debt round's review, then merging
is the owner's word; ART-322 is the owner's call. None of the items below are it.
