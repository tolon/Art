...
| **i18n** | `en.json` and `tr.json` on `main`: **2026** leaf keys each, counted 2026-09-06 after the media-hash merge (`800fdc0`) — **2012** before it, after the refusal-evidence merge (`bd91942`) — **2009** before it, after the drawer-icons merge (`722a8c5`) — up from **2005** at item 7's merge (`bf5bbe7`), **1972** after the WHDLoad-drawer round, 1958 earlier that same day, 1916 before the layered-release round. On the unmerged `art-refusal-evidence` branch: **2012**, up from `main`'s 2009 — the three `osinstall.evidence.*` keys. The 2008 this row carried for the drawer-icons branch was wrong and contradicted the branch row's own 2009; both were re-counted rather than reconciled. On the unmerged `art-media-hash` branch (built on top of `main`'s 2012, after the refusal-evidence merge): **2026** after fix wave 1, up from **2021** at the round's own end — nine `osinstall.mediaId.*` keys from the round plus five more from ART-263/ART-264 (`cancelled` and the four per-folder results), both catalogues in sync. Count them, do not quote this; parity enforced by `pnpm test`. **On the unmerged `art-firstboot` branch (round 5, first-boot phases 1–2)**: **2076** leaf keys each catalogue, counted 2026-09-07 at Task 12 — up from 2026 at the branch's start; growth came from Task 9's OS Builder step and Task 11's card-report `source` keys. **After the whole-branch review's fix wave, same day: 2078** — C1's `firstboot.panel.copyDiscarded` and I4's `firstboot.step.refusedPlain`, both catalogues, both counted by `pnpm test` rather than quoted here |
...
1. **`main` is still unpushed — 39 commits ahead of `origin/main`, two local
   merges (round 3 `bd91942` and round 4 `800fdc0`) that CI has never seen —
   and this session did not touch `main` at all.** All of this session's work
   is on `art-firstboot`, branched from `main` at `0bdce5d`. **Do not assume
   `origin/main` matches either tree.**
2. **Round 5 of the Emu68 Hatcher intake — the first-boot mechanism — has its
   phases 1 and 2 built, reviewed and documented on `art-firstboot`, and
   NOT merged to `main`.** Twelve tasks: the spike (Task 1), the AmigaDOS
   ...
   AmigaDOS is still executing it (ART-273). **Merging `art-firstboot` into
   `main` and pushing are the owner's to do — neither happened this session.**
...
Standing numbers on `main` are unchanged by this session: **83 frontend files
/ 1119 tests**; Rust **2878 passed, 0 failed, 50 ignored** (`--skip
artwork`); i18n **2026** leaf keys each catalogue; **18 open** entries in
[ISSUES.md](ISSUES.md). On `art-firstboot` (branch-only, see the Snapshot's
Tests and i18n rows above for the full quotes): **87 frontend files / 1173
tests**; Rust **2945 passed, 0 failed, 51 ignored** (`--skip artwork`, run
twice); i18n **2076** leaf keys each catalogue; ISSUES.md gained two entries
this round (ART-272, ART-273, both filed and fixed the same day) and two
existing entries (ART-166, ART-168) gained a paragraph each rather than a
new number.
