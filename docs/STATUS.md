...
| **Tests — Rust** | `cd src-tauri && cargo test --lib` — the **full** suite — on `art-091-fixes`, 2026-09-09: `test result: ok. 3186 passed; 0 failed; 53 ignored` (run twice, ART-059). It was 3 227 the day before, on the same branch: the BoingBag cleanup removed the emulator route for the two BoingBags and every guard that only covered it, and added six of its own. On the merged `main` (`e0fb4aa`) on 2026-09-08 it was `3202 passed; 0 failed; 52 ignored` |
| **Tests — frontend** | `pnpm test` on `art-091-fixes`, 2026-09-09: `Test Files 91 passed (91)`, `Tests 1389 passed (1389)` (1 386 before fix round 1's two ART-286 cases; 1 388 before fix round 2's ART-287 case) — 1 410 the day before, on the same branch. On the merged `main` (`e0fb4aa`), 2026-09-08: 91 / 1410 |
