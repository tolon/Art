...
lint and four sweeps clean. **Still true from below:** the next run's `JOURNAL-PENDING` refusal says "undo it first"
for any journal, including one I2's ending tells the user to delete. **Next:** push and watch CI once, then the
owner's merge word.
