...
**Fixed 2026-09-07** — not in the repository: the owner added the antivirus
exclusions (`src-tauri\target\`, the test scratch root and `art_lib`'s build
output). Measured the same evening on the merged `main` (`7825549`), the
full suite, no `--skip`, twice: `cargo test --lib` → `test result: ok. 3062
passed; 0 failed; 51 ignored; 0 measured; 0 filtered out; finished in 41.36s`,
then `... finished in 43.90s` — the 89 `artwork` tests that used to kill the
harness now run and pass, and the summary line is printed both times. The
guard is the rule this entry produced, not a test: **quote the `test result:`
line, never the exit code** (`docs/lessons.md`, `CLAUDE.md` § Before you
commit). `--skip artwork` is no longer needed on this machine; STATUS's
reproduce block quotes the full suite again.
...
**Fixed 2026-08-24.** `core/archive/compress.rs` — a `compress(1)` LZW reader,
read only, written rather than depended on: `.Z` is small and fully specified,
and a crate would have joined `cargo deny`'s surface for about a hundred lines
while a decompressor pointed at untrusted bytes is exactly the thing whose
bounds want to be visible. Every one of them is: the code width is checked
against the header's own maximum, the dictionary cannot grow past it, an
undefined code is refused by name, and the output is capped at 64 MB.

`plan` marks an item `decompress` when the medium's name ends `.Z` and strips
the suffix from the destination — because the release drops it too, so the
tree holds `dos.catalog` and the manifest records that name and the hash of
those bytes. `apply` expands on the way in.

**Checked against a tool that is not ART.** `assign.help` off the owner's own
`Locale-EN.adf`: ART writes 8 216 bytes with SHA-256
`93043d551fc9465a8104ea41…`, and 7-Zip decompressing the same source produces
the same bytes and the same hash. A permanent hook
(`compress::oracle_hook::matches_7zip_on_a_real_file`, `#[ignore]`d,
env-gated) does that comparison on any `.Z` it is pointed at.

**Measured on the owner's own 3.2 ADFs, both ROMs.** `compressed items=3263`
either way, 0 refusals, and **no `.Z` left in the tree**:

| paired Kickstart | files | dirs | bytes |
|---|---|---|---|
| V40 (`40.68` A1200) | 3 976 | 281 | **19 839 113** (was 12 751 572) |
| V47 (3.2's own `kicka1200.rom`) | 3 972 | 278 | **19 789 593** (was 12 702 052) |

Seven megabytes of content that used to reach the volume packed and misnamed.
`Locale/Help/english/Sys/Commands` now holds `addbuffers.help`,
`adddatatypes.help`, `assign.help` — the names AmigaOS looks for.

**What it cost, said plainly.** `planned.total_bytes` and `outcome.bytes` no
longer agree for a tree with compressed content, and [ART-156](#fixed)
established that they should. A `.Z` stream carries no expanded length, so the
only way for the plan to predict it is to expand every file — and `plan()`
runs live while a person ticks boxes, so that price would be paid on every
keystroke. `PlanItem::bytes` stays the medium's own figure, the new
`decompress` field says when the two differ, and the hook pins what `apply`
actually wrote. This is a loss, not a wash.

**The one path with no test, disclosed rather than claimed.** A `compress`
stream may reset its dictionary mid-way, and that branch is implemented from
the specification and exercised by nothing. No file on the owner's media gets
near it — a reset happens when the dictionary fills, 65 536 entries at the
16-bit maximum these files declare, and the largest `.Z` here is a few
kilobytes. A fixture was attempted with a hand-written encoder emitting a
clear code, and **7-Zip refused to agree with it**, which says the encoder's
padding was wrong and says nothing about the decoder's. Tuning the encoder
against 7-Zip and then testing the decoder against the tuned encoder is a
circle wearing an oracle's clothes, so it was not done. The branch is written
because a stream that resets and is silently truncated would be worse than one
that is untested.

**The manifest question this entry raised is answered by not arising.** The
worry was that recording the placed bytes' hash would describe something that
is not on the medium. It records what is *in the tree*, which is what `verify`
reads back and what a person is asking about; the medium is named in the same
row. Nothing is lost, so nothing was added.

**Still not measured: whether AmigaOS 3.9 ships any `.Z`.** Its recipe reads a
CD rather than a floppy set and may well carry those files plain. The hook now
prints `compressed items=`, so the next 3.9 run answers it for free.
...
`src-tauri/src/core/osinstall/recipe.rs::release_holding`

The owner chose AmigaOS 3.2 while the media folder still held the AmigaOS 3.9
disc from the previous session ([ART-207](#fixed) is why it was still there).
`plan()` answered correctly — sixteen components, sixteen `MediaMissing`
refusals:

```
workbench-base, Workbench3.2 istiyor ve medya klasöründe bu birim adını taşıyan bir dosya yok.
install-libs, Install3.2 istiyor ve medya klasöründe bu birim adını taşıyan bir dosya yok.
… fourteen more
```

Every sentence true. What the owner read off the screen was **"bir sürü eksik
program"** — a lot of missing programs — and went looking for disks that were
never missing. This is the "failure that does not crash" exactly: nothing
logged, nothing red, a confident screen and a wrong conclusion.

**ART had everything needed to say it once.** It had just read the folder and
knew what was in it (`AmigaOS3.9`), it knew what the recipe wanted, and it
ships the 3.9 recipe too — so it can name the release the folder actually
belongs to and offer the switch.

**Fixed 2026-08-22.** `recipe::release_holding` answers "whose media is this?"
from the shipped recipes, comparing with `amiga_names_equal` so it agrees with
the `scan::media_for` that produced the refusals; two matching releases answer
`None`, because "both" is not something a one-click switch can act on.
`wrongMediaFolder` decides when the folder — not the disks — is the fact, and
`osinstallBlocker` says it beside the button that is blocked, with the refusals
card suppressed so the sentence appears **once** (ART-202's rule, in the
owner's own words: *"aynı uyarı tek ekranda 2 tane"*).

**The claim is checked, not inferred.** The first version fired on "no plan
items and every refusal about media", and the existing suite caught it: a
fixture whose folder held `Workbench3.2` while a refusal named `Workbench3.2`
absent was told its folder was somebody else's. It now requires that **not one
refused disk is in the folder**, which is what the sentence actually claims.
The JS side folds case with `toLowerCase` rather than AmigaDOS's own folding,
and the direction is why that is safe: these are names Rust already refused to
match, so a looser fold can only withdraw the sentence in favour of the
per-disk list, never produce it wrongly.

Tests: `recipe::tests::names_the_release_whose_media_a_folder_actually_holds`,
`names_the_other_release_the_same_way_round`,
`names_no_release_for_media_no_recipe_asks_for`,
`names_no_release_when_the_folder_holds_two_releases_media`,
`names_no_release_for_an_empty_folder`,
`a_folder_is_identified_the_way_its_disks_are_matched`; `osinstallBlocker`'s
seven cases in `src/lib/osinstall.test.ts`; and on the screen,
`says it once, instead of once per component`,
...
Mutations run and fallen: the refusals card left unsuppressed (the sentence
renders twice — the owner's own ART-202 complaint), the switch button removed,
the folder-contents check removed (three tests, including the pre-existing
`shows the real, translated refusal text`), and `amiga_names_equal` swapped for
`==` (the case-fold test).

**ART-207** 🟠 ✅ **A media folder outlived the release it belonged to** —
*found 2026-08-22 by the owner, driving the OS Builder*
`src/components/osbuilder/OsInstall.tsx`

`chosen` and `excludedConditional` have been remembered **per release** since
the picker arrived, and the reason is in that code's own comment: *a component
id means something only inside the recipe that declares it.* The media folder
and the destination were remembered in one key each, for all releases.

That sentence is true word for word of a folder. The owner keeps AmigaOS 3.9's
disc in `Amigatolon\iso` and 3.2's ADFs in `Amigatolon\paketler`; no folder can
serve both. So switching the release changed the component list correctly and
left behind a folder that could not possibly match — which is how they reached
[ART-208](#fixed)'s screen. **The two are one story: this is the cause, and
that is what it looked like.**

**Fixed 2026-08-22** — `rememberedComponentKey` now keys `osinstall.mediaFolder`
and `osinstall.destination` as well, so each release recalls its own. That
function already answers the migration: for AmigaOS 3.2, "the release before
there was a picker", the key stays the bare one, so nothing anybody stored has
moved or been lost.

**The ROM is deliberately not part of it.** A Kickstart is a property of the
machine being built for, not of the release being installed: the owner's one
licensed A1200 ROM is the right answer for 3.2 and 3.9 alike, and asking for it
again per release would be a choice resetting itself for no reason anybody
could state.

Tests: `shows each release's own media folder, never the other's`,
`plans a release into its own destination, not the other release's`,
`keeps each release's own media folder when the user switches away and back`.
...
**Fixed 2026-08-22.** `merge_config_txt` is section-aware: it tracks the
current `[…]` header and keys its written-set on **`(section, key)`**, so the
same key in two sections is two settings rather than one written twice.

**And a file that boots per board keeps its own lines.** `selects_boot_per_board`
answers whether the file carries any `[gpio…]` stanza. When it does, ART leaves
every `kernel=` and every `initramfs` exactly as the release wrote them — the
release knows its own boot layout and ART, holding one `kernel_file`, does not.
ART's single kernel name is for the other case: a flat file, or one written
from nothing. **The Kickstart is still named**, appended when no existing line
already names it, because keeping a release's lines must not mean refusing to
add ART's.

**Measured against the real file, before and after:**

| | before | after |
|---|---|---|
| `kernel=` lines | **1**, in the stealth stanza | **4**, each in its own |
| `[gpio…]` stanzas with a kernel | 1 of 4 | 4 of 4 |
| stealth `initramfs ps32lite-stealth-firmware.gz` | replaced by the ROM | **kept** |
| Kickstart named | yes | yes |

**The fixture is the structure, and that is the point.** Every `config.txt`
this module was tested against before was a flat file ART had written itself —
a reader and a writer agreeing with each other, which is the shape that already
cost [ART-032](#fixed)…[ART-035](#fixed) and [ART-079](#fixed). The new fixture
reproduces a real release's sections, keys and order.

Tests: `every_board_keeps_its_own_kernel`, `every_gpio_stanza_survives`,
`a_stanzas_own_initramfs_is_not_replaced_by_the_kickstart`,
`a_managed_key_is_rewritten_once_in_each_section_that_has_it`,
`a_flat_config_still_behaves_exactly_as_before`,
`a_sectioned_file_still_gains_the_kickstart_it_had_no_line_for`.

**Three mutations, three kills**: remove the per-board `kernel` guard and
`every_board_keeps_its_own_kernel` fails; drop the section from the written-key
and `a_managed_key_is_rewritten_once_in_each_section_that_has_it` fails; delete
the per-board `initramfs` preservation and
`a_stanzas_own_initramfs_is_not_replaced_by_the_kickstart` fails.

**What this does NOT establish, said plainly.** A card ART builds has still
never been booted. This fix stops `config.txt` losing lines it was destroying;
it does not prove the result boots. One thing in particular is unsettled: ART
appends `initramfs <rom>` in the file's **unconditional** tail, while the Emu68
Imager substitutes a `[ROMPATH]` marker **inside each board stanza**. Whether
the Pi's firmware resolves the ROM the same way from both positions is a
question only a real boot answers, and it is the next thing to find out.
