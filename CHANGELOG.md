...
- **AmigaOS 3.9: the Turkish ISO-8859-9 font set.** A new optional component
  ...
  assumed.
- **AmigaOS 3.2: the help system is readable now.** Your 3.2 disks keep most
  of their Locale content compressed, and the real installer expands it while
  copying. ART did not: 3 263 files — every help page, in every language, plus
  the Turkish fonts and catalogs — reached the volume still packed and under
  the wrong name, so AmigaOS never found them. They are expanded now, which
  makes an installed tree about seven megabytes larger and actually complete.
